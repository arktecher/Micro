...
import { ArrowLeft, Package, Calendar, AlertCircle, CheckCircle2, TruckIcon, Loader2, } from "lucide-react";
import { getReturnRequestContext, createReturnRequest, type ReturnRequestContext, } from "@/services/space.service";
...
export function ArtworkReturnRequestPage() {
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        {!ctx.can_submit && ctx.pending_message && (<div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
            {ctx.pending_message}
          </div>)}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <div className="w-full sm:w-40 h-40 rounded-lg overflow-hidden flex-shrink-0">
                    <ImageWithFallback src={ctx.main_image_url ||
            "https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=800&h=600&fit=crop"} alt={ctx.artwork_title || "作品"} className="w-full h-full object-cover"/>
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
            <Card>
              ...
              <CardContent>
                <Textarea value={additionalComments} onChange={(e) => setAdditionalComments(e.target.value)} placeholder="例：次回は明るい色調の作品を希望します" className="min-h-[120px] resize-none"/>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="flex flex-col sm:flex-row gap-4 justify-end">
            ...
            !agreedToTerms ||
            isSubmitting ||
            !ctx.can_submit} className="bg-accent hover:bg-accent/90 min-w-[160px] w-full sm:w-auto">
