...
function buildArtworkImageUrls(a: Artwork): string[] {
  ...
  if (a.main_image_url?.trim()) return [a.main_image_url];
  return [];
}

function buildSpaceImageUrls(s: SpaceResponse): string[] {
  const urls = (s.photo_urls ?? []).filter(
...
interface ImageCarouselProps {
  images: string[];
  altPrefix: string;
  /** Tailwind aspect class e.g. aspect-square, aspect-video */
  aspectClass?: string;
  className?: string;
  /** Narrow list row: smaller controls */
  compact?: boolean;
}
...
function ImageCarousel({
  ...
  if (n === 0) {
    return (
      <div
        className={`rounded-lg bg-gray-100 ${aspectClass} ${className}`}
        aria-hidden
      />
    );
  }
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-lg bg-gray-100 ${aspectClass} ${className}`}
    >
      <ImageWithFallback
        src={list[safeIdx]}
        alt={`${altPrefix} ${safeIdx + 1}`}
      ...
      {n > 1 && (
        <>
          <button
            ...
            className={`absolute left-1 top-1/2 z-10 -translate-y-1/2 rounded-full border border-white/80 bg-white/90 text-gray-800 shadow hover:bg-white ${
              compact ? "p-0.5" : "p-1"
            }`}
            aria-label="前の画像"
          >
          ...
          <button
            ...
            className={`absolute right-1 top-1/2 z-10 -translate-y-1/2 rounded-full border border-white/80 bg-white/90 text-gray-800 shadow hover:bg-white ${
              compact ? "p-0.5" : "p-1"
            }`}
            aria-label="次の画像"
          >
...
/** 別作品の展示依頼〜受領前まで、このスペースに新規の展示依頼は出せない */
function spaceHasActiveExhibitionPipeline(s: SpaceResponse): boolean {
  return Boolean(
    s.pending_exhibition_artwork_id ||
...
export function CorporateDisplayRequestPage() {
  ...
  const preSelectedSpaceId = (location.state as { spaceId?: string } | null)
    ?.spaceId;
  ...
  useEffect(() => {
    ...
    artworkService
      ...
      .catch((e: unknown) => {
        if (!cancelled) {
          setArtworkError(
            e instanceof Error ? e.message : "作品を読み込めませんでした"
          );
          setArtwork(null);
        }
      })
  ...
  useEffect(() => {
    ...
    listSpaces({ page: 1, page_size: 100, is_active: true })
      .then((res) => {
        if (!cancelled) setSpaces(res.items ?? []);
      })
      .catch((e: unknown) => {
        if (!cancelled) {
          setSpacesError(
            e instanceof Error ? e.message : "スペース一覧の取得に失敗しました"
          );
          setSpaces([]);
        }
      })
  ...
  useEffect(() => {
    if (!preSelectedSpaceId || !spaces.length) return;
  ...
  const artworkImageUrls = useMemo(
    () => (artwork ? buildArtworkImageUrls(artwork) : []),
    [artwork]
  );
  const selectedSpaceImageUrls = useMemo(
    () => (selectedSpace ? buildSpaceImageUrls(selectedSpace) : []),
    [selectedSpace]
  );
  ...
  if (!isAuthenticated || userType !== "corporate") {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#FAFAFA] to-white px-4 py-24">
  ...
  if (corporateRole === null) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#FAFAFA] gap-3">
        <Loader2 className="h-10 w-10 animate-spin text-primary" />
  ...
  // API returns status "exhibited" when the work has an active space assignment
  // (DB row is still "published"); both are eligible for corporate space assignment.
  const canAssignToSpace =
    artwork.status === "published" || artwork.status === "exhibited";

  if (!canAssignToSpace) {
    return (
      <div className="min-h-screen bg-white px-4 py-24 text-center max-w-lg mx-auto">
                    ...
                    <form onSubmit={onSubmitStep1} className="space-y-6">
                      ...
                      >
                        <Card>
                          <CardContent className="p-6">
                            ...
                            {spacesError && (
                              <p className="text-sm text-red-600 py-4">{spacesError}</p>
                            )}
                            ...
                            {!spacesLoading && spaces.length > 0 && (
                              <div className="space-y-3">
                                {spaces.filter((space) =>
                                  !space.current_artwork_id &&
                                  !spaceHasActiveExhibitionPipeline(space)
                                ).length === 0 && (
                                  <div className="text-center py-8">
                                    ...
                                    >
                                      <div className="flex gap-4 sm:gap-5">
                                        <div className="h-28 w-40 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100 sm:h-36 sm:w-52">
                                          <ImageCarousel
                                            images={buildSpaceImageUrls(space)}
                      ...
                      <div className="flex justify-end">
                        <Button
                          ...
                          disabled={!selectedSpaceId || spacesLoading}
                        >
                          確認画面へ
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="mx-auto w-full max-w-[280px] flex-shrink-0 sm:mx-0 sm:w-40">
                          <ImageCarousel
                            images={selectedSpaceImageUrls}
                            altPrefix={selectedSpace.name}
                            aspectClass="aspect-square"
                          />
                        </div>
                      </div>
