...
import { Building2, MapPin, Mail, Phone, Settings, Shield, Wallet, ChevronLeft, Bell, Lock, History, Loader2, Users, UserPlus, Copy, Trash2, RefreshCw, } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";
import { useCorporateOrgRole } from "@/hooks/useCorporateOrgRole";
import { userService, type UserProfile } from "@/services/user.service";
import { corporateInvitationsService, type CorporateInvitation, type CorporateInviteRole, } from "@/services/corporateInvitations.service";
...
export function CorporateProfilePage() {
    ...
    const { canAdmin } = useCorporateOrgRole((profile?.corporate_role as CorporateOrgRole | undefined) ?? null);
    useEffect(() => {
        if (!isInitialized)
            return;
    ...
    const handleSubmitInvite = async () => {
        ...
        try {
            ...
            if (created.email_sent === false) {
                toast.info("招待メールは送信されませんでした。.env の RESEND_API_KEY と EMAIL_FROM（Resend で検証済み）を確認するか、SMTP を使う場合は SMTP_HOST を設定してください。リンクの手動共有も可能です。");
            }
            setInviteDialogOpen(false);
    ...
    return (<div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <div className="flex min-h-0 flex-1 flex-col pt-16 sm:pt-20">
        ...
        <div className="container mx-auto flex-1 px-4 py-8 sm:px-6">
          {loading ? (<div className="flex flex-col items-center justify-center gap-3 py-24">
              <Loader2 className="h-10 w-10 animate-spin text-primary"/>
              <p className="text-sm text-gray-600">プロフィールを読み込み中…</p>
            </div>) : loadError ? (<Card className="max-w-lg border-destructive/30">
              <CardHeader>
              ...
              <div className="space-y-6 lg:col-span-2">
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {canAdmin ? (<Button variant="outline" size="sm" onClick={() => {
                    ...
                    <CardContent className="space-y-4">
                      {isEditingCompany ? (<div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Input id="company_name" value={companyName} onChange={(e) => setCompanyName(e.target.value)} autoComplete="organization"/>
                          </div>
                          <div className="space-y-2">
                            <Label htmlFor="contact_name">担当者名</Label>
                            <Input id="contact_name" value={contactName} onChange={(e) => setContactName(e.target.value)} autoComplete="name"/>
                          </div>
                          <div className="space-y-2">
                          ...
                          <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input id="postal_code" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="例：1500001"/>
                            </div>
                          </div>
                          <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2 pt-2">
                            <Button className="bg-primary hover:bg-primary/90" disabled={saving} onClick={() => void handleSaveCompany()}>
                              {saving ? (<>
                          ...
                          {profile?.profile_completion != null && (<p className="text-xs text-gray-500">
                ...
                <motion.div ref={paymentSectionRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }}>
                  <Card>
                    <CardHeader>
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        {canAdmin ? (<div className="flex shrink-0 gap-2">
                            <Button type="button" variant="outline" size="sm" disabled={invitesLoading} onClick={() => void loadInvitations()} aria-label="招待一覧を更新">
                              <RefreshCw className={`mr-1 h-4 w-4 ${invitesLoading ? "animate-spin" : ""}`}/>
                              更新
                            </Button>
                            <Button type="button" size="sm" className="bg-primary hover:bg-primary/90" onClick={handleOpenInvite}>
                    ...
                    <CardContent className="space-y-3">
                      {!canAdmin ? (<p className="text-sm text-gray-600">
                          チームの招待・一覧の管理は<strong className="text-foreground">管理者</strong>
                          （アカウント代表または招待された管理者）のみが行えます。
                        </p>) : invitesLoading && invitations.length === 0 ? (<div className="flex items-center gap-2 py-6 text-sm text-gray-600">
                          <Loader2 className="h-5 w-5 animate-spin text-primary"/>
                          招待一覧を読み込み中…
                        </div>) : invitations.length === 0 ? (<p className="text-sm text-gray-600">
                          まだ招待がありません。「招待する」からメールアドレスと権限を指定してください。
                        </p>) : (<ul className="space-y-3">
                          {invitations.map((inv) => (<li key={inv.id} className="rounded-lg border bg-white p-3 text-sm shadow-sm">
                              <div className="flex flex-wrap items-start justify-between gap-2">
                                <div className="min-w-0 flex-1 space-y-1">
                                  ...
                                  <div className="flex flex-wrap items-center gap-2 text-xs text-gray-600">
                                    ...
                                    {inv.job_title ? (<span className="text-gray-500">· {inv.job_title}</span>) : null}
                                  </div>
                              ...
                              {inv.status === "pending" && inv.invite_url ? (<div className="mt-3 flex flex-wrap gap-2 border-t pt-3">
                                  <Button type="button" variant="outline" size="sm" onClick={() => void copyInviteUrl(inv.invite_url!)}>
                                    <Copy className="mr-1 h-4 w-4"/>
                                    リンクをコピー
                                  </Button>
                                  <Button type="button" variant="outline" size="sm" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => void handleRevokeInvite(inv.id)}>
      ...
      <Dialog open={inviteDialogOpen && canAdmin} onOpenChange={(open) => {
            if (!canAdmin)
                return;
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-2">
            <div className="space-y-2">
              ...
              <Input id="invite_email" type="email" autoComplete="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="member@example.com"/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="invite_role">権限</Label>
              <Select value={inviteRole} onValueChange={(v) => setInviteRole(v as CorporateInviteRole)}>
            ...
            <div className="space-y-2">
              ...
              <Input id="invite_job_title" value={inviteJobTitle} onChange={(e) => setInviteJobTitle(e.target.value)} placeholder="例：展示担当" maxLength={200}/>
            </div>
          </div>
          <DialogFooter className="gap-3">
            ...
            <Button type="button" className="bg-primary hover:bg-primary/90" disabled={inviteSubmitting} onClick={() => void handleSubmitInvite()}>
              {inviteSubmitting ? (<>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                  送信中…
                </>) : ("招待を作成")}
