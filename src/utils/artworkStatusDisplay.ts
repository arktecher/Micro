/**
 * User-facing labels for artwork `status` (API / DB values).
 * Granular in-transit / returned statuses are grouped for display where appropriate.
 */
...
export function artworkStatusLabelJa(
  status: string | undefined | null,
): { label: string; variant: ArtworkStatusDisplayVariant } {
  const s = (status || "").trim();
...
export function badgeClassForArtworkStatusVariant(
