...
export function ArtworkCard({ artwork, onClick }: ArtworkCardProps) {
    ...
    return (<>
      <Card className="group cursor-pointer overflow-hidden rounded-xl border border-gray-200/90 bg-white shadow-sm transition-all duration-300 hover:border-gray-300 hover:shadow-md" onClick={onClick}>
        ...
        <div className="relative aspect-square bg-gray-100 overflow-hidden">
          ...
          {artwork.status === "rented" && (<div className="absolute top-2 left-2 sm:top-4 sm:left-4">
              <Badge className="bg-orange-500 text-white text-xs">
                貸出中
              </Badge>
            </div>)}
          {artwork.status === "displayed" && (<div className="absolute top-2 left-2 sm:top-4 sm:left-4">
          ...
          <button onClick={handleLikeClick} className="absolute bottom-2 right-2 sm:bottom-4 sm:right-4 w-8 h-8 sm:w-10 sm:h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition-all z-10">
            <Heart className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${isLiked
        ...
        <div className="p-3 sm:p-4 md:p-6 space-y-2 sm:space-y-3">
          ...
          <div className="flex items-center justify-between pt-1 sm:pt-2">
            ...
            {!isArtistUser && (<Button variant="outline" size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity hidden sm:flex text-xs sm:text-sm">
                詳細を見る
              </Button>)}
          </div>
        </div>
      ...
      <SignupPromptDialog isOpen={showSignupPrompt} onClose={() => setShowSignupPrompt(false)} artworkId={artwork.id} artworkTitle={artwork.title}/>
    </>);
}
