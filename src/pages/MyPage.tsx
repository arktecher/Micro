...
import { User, MapPin, CreditCard, ShoppingBag, Heart, Sparkles, ChevronLeft, ChevronRight, Package } from "lucide-react";
...
function ArtworkCard({ artwork, showDate = false, onRemoveFavorite }: {
    artwork: any;
    showDate?: boolean;
    onRemoveFavorite?: (id: string) => void;
}) {
    const navigate = useNavigate();
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="group cursor-pointer" onClick={() => navigate(`/artwork/${artwork.id}`)}>
      <div className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
        <div className="aspect-square relative overflow-hidden bg-gray-100">
          <ImageWithFallback src={artwork.image} alt={artwork.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"/>
          
          {onRemoveFavorite && (<button onClick={(e) => {
                e.stopPropagation();
        ...
        <div className="p-4 space-y-2">
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            {showDate && artwork.purchaseDate && (<p className="text-xs text-gray-400">{artwork.purchaseDate}</p>)}
          </div>
        </div>
      </div>
    </motion.div>);
}
function RecommendedCarousel({ artworks }: {
    artworks: any[];
}) {
    ...
    return (<div className="relative group">
      ...
      {currentIndex > 0 && (<button onClick={handlePrev} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-6 z-10 w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 opacity-0 group-hover:opacity-100">
      ...
      <div className="overflow-hidden">
        <motion.div className="flex gap-6" animate={{
          ...
          {artworks.map((artwork) => (<div key={artwork.id} className="flex-shrink-0" style={{ width: `calc(25% - 1.125rem)` }}>
      ...
      {currentIndex < maxIndex && (<button onClick={handleNext} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-6 z-10 w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center hover:bg-white hover:scale-110 transition-all duration-300 opacity-0 group-hover:opacity-100">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div>
                    ...
                    <Input id="name" value={profileData.name} onChange={(e) => setProfileData({ ...profileData, name: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="email" type="email" value={profileData.email} readOnly className="h-10 sm:h-11 bg-gray-100 border-gray-200 text-gray-500 cursor-not-allowed"/>
                  </div>
                </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div>
                    ...
                    <Input id="shipping-name" value={shippingData.name} onChange={(e) => setShippingData({ ...shippingData, name: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="phone" value={shippingData.phone} onChange={(e) => setShippingData({ ...shippingData, phone: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="postal-code" value={shippingData.postalCode} onChange={(e) => setShippingData({ ...shippingData, postalCode: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="prefecture" value={shippingData.prefecture} onChange={(e) => setShippingData({ ...shippingData, prefecture: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="city" value={shippingData.city} onChange={(e) => setShippingData({ ...shippingData, city: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                  ...
                  <div>
                    ...
                    <Input id="address" value={shippingData.address} onChange={(e) => setShippingData({ ...shippingData, address: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Input id="building" value={shippingData.building} onChange={(e) => setShippingData({ ...shippingData, building: e.target.value })} className="h-10 sm:h-11 bg-[#F5F5F5] border-gray-200 focus:border-[#C6A05C] transition-all"/>
                </div>
          ...
          <TabsContent value="favorites" className="space-y-8 sm:space-y-12">
            <div>
              ...
              {favoriteArtworks.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  {favoriteArtworks.map((artwork) => (<ArtworkCard key={artwork.id} artwork={artwork} onRemoveFavorite={handleRemoveFavorite}/>))}
                </div>) : (<Card className="p-12 sm:p-16 rounded-lg shadow-sm border border-gray-200/50">
          ...
          <TabsContent value="purchases">
            ...
            {mockPurchasedArtworks.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                {mockPurchasedArtworks.map((artwork) => (<ArtworkCard key={artwork.id} artwork={artwork} showDate={true}/>))}
              </div>) : (<Card className="p-12 sm:p-16 rounded-lg shadow-sm border border-gray-200/50">
                <div className="text-center space-y-4">
