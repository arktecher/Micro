...
import { ImageIcon, Video, Eye, MapPin, TrendingUp, Wallet, User, Edit, ExternalLink, Calendar, BarChart3, Sparkles, Heart, Package, CheckCircle, Clock, DollarSign, QrCode, Plus, Building2, RotateCcw, ChevronLeft, ChevronRight, Search, Filter, X, ChevronDown, ChevronUp, AlertTriangle, } from "lucide-react";
...
import { artistService, type ArtistProfile, type ArtistStatistics, type RevenueAnalytics, } from "@/services/artist.service";
import { userService } from "@/services/user.service";
import { artworkService, type Artwork as ArtworkAPI, type ArtistArtworkStatusCounts, isArtworkInTransitFamily, } from "@/services/artwork.service";
...
function FilterChipCount({ n, active }: {
    n: number;
    active: boolean;
}) {
...
export function ArtistDashboard() {
    ...
    const [artworkCarousels, setArtworkCarousels] = useState<Map<string, {
        currentIndex: number;
    ...
    useEffect(() => {
        if (selectedTab !== "artworks" || artworkFilter !== "returned")
            return;
    ...
    useEffect(() => {
        if (selectedTab !== "revenue" || userType !== "artist" || !isAuthenticated)
            return;
        let cancelled = false;
    ...
    useEffect(() => {
        ...
        if (artworkFilter && artworkFilter !== "all") {
            const statusParam = artworkFilter === ARTWORK_FILTER_IN_TRANSIT_UNIFIED
                ? ARTWORK_FILTER_IN_TRANSIT_UNIFIED
                : artworkFilter;
        ...
        if (filters.hasFrame === true)
            params.set("has_frame", "true");
    ...
    useEffect(() => {
        ...
        if (statusParam) {
            const legacyTransit = statusParam === "on_the_way" ||
            ...
            const filter = statusParam === "recalled"
                ? "all"
    ...
    const hasProfileChanges = () => {
        return (profileFormData.name !== originalProfileData.name ||
    ...
    const loadArtworks = async () => {
        ...
        const isInTransitUnifiedFilter = artworkFilter === ARTWORK_FILTER_IN_TRANSIT_UNIFIED;
        ...
        try {
            ...
            const newCarousels = new Map<string, {
                currentIndex: number;
                isAutoPlaying: boolean;
                isHovering: boolean;
            }>();
    ...
    useRefreshOnInterval(() => {
        if (selectedTab !== "artworks" || !currentUser?.id)
            return;
        void loadArtworks();
    }, selectedTab === "artworks" && Boolean(currentUser?.id), 45000);
    useEffect(() => {
        const intervals: Map<string, NodeJS.Timeout> = new Map();
        artworkCarousels.forEach((carouselState, artworkId) => {
            const artwork = artworks.find(a => a.id === artworkId);
            if (!artwork || !artwork.images || artwork.images.length <= 1)
                return;
    ...
    const handleSaveProfile = async () => {
        try {
            ...
            const allShippingRequiredFilled = shippingRequired.every((v) => (v || "").trim().length > 0);
            if (anyShippingFilled && !allShippingRequiredFilled) {
                toast.error("配送・返送先は、郵便番号・都道府県・市区町村・番地をすべて入力するか、すべて空にしてください。");
                setIsSavingProfile(false);
                return;
            }
            if (allShippingRequiredFilled && !profileFormData.name.trim()) {
                toast.error("配送先を保存するには、先に「名前（公開名）」を入力してください（宛名として使用されます）。");
                setIsSavingProfile(false);
                return;
            }
    ...
    const publishedCount = statusCounts?.published ?? artistStatistics?.artwork_counts.published ?? 0;
    const exhibitedCount = statusCounts?.exhibited ?? artistStatistics?.artwork_counts.exhibited ?? 0;
    ...
    const qrMonthlyPercent = qrTotal > 0 ? Math.min(100, Math.round((qrMonthly / qrTotal) * 100)) : 0;
    const revenueByMonth = revenueAnalytics?.revenue_by_period ?? [];
    const maxMonthRev = Math.max(...revenueByMonth.map((r) => r.revenue), 1);
    ...
    return (<div className="min-h-screen bg-[#F8F6F1]">
        ...
        <Tabs value={selectedTab} onValueChange={handleTabChange} className="w-full">
          <div className="mb-6 sm:mb-8 overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
            <TabsList className="grid w-full grid-cols-4 sm:grid-cols-4 min-w-[600px] sm:min-w-0 bg-white p-1 rounded-2xl shadow-sm">
              <TabsTrigger value="dashboard" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#C3A36D] data-[state=active]:to-[#D4B478] data-[state=active]:text-white rounded-xl text-xs sm:text-sm">
                <BarChart3 className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2"/>
                <span className="whitespace-nowrap">ダッシュボード</span>
              </TabsTrigger>
              <TabsTrigger value="artworks" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#C3A36D] data-[state=active]:to-[#D4B478] data-[state=active]:text-white rounded-xl text-xs sm:text-sm">
                <ImageIcon className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2"/>
                <span className="whitespace-nowrap">作品一覧</span>
              </TabsTrigger>
              <TabsTrigger value="profile" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#C3A36D] data-[state=active]:to-[#D4B478] data-[state=active]:text-white rounded-xl text-xs sm:text-sm">
                <User className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2"/>
                <span className="whitespace-nowrap">プロフィール</span>
              </TabsTrigger>
              <TabsTrigger value="revenue" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#C3A36D] data-[state=active]:to-[#D4B478] data-[state=active]:text-white rounded-xl text-xs sm:text-sm">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <Card className="bg-white border-2 border-[#C3A36D]/30 h-full">
                  <CardContent className="pt-6 h-full flex flex-col">
                    ...
                    {qrTopLocations.length > 0 && (<p className="text-xs text-gray-500 mt-2">
              ...
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Progress value={qrTotal > 0 ? 100 : 0} className="h-2"/>
                  </div>
                  <div className="pt-4 border-t">
                    <p className="text-sm text-gray-600 mb-3">📍 スキャンが多い展示スペース</p>
                    <div className="space-y-2">
                      {qrTopLocations.length === 0 ? (<p className="text-sm text-gray-500">
                          まだQRスキャンがありません。展示が始まると集計されます。
                        </p>) : (qrTopLocations.map((loc) => (<div key={loc.space_id} className="flex items-center justify-between p-3 bg-[#F8F6F1] rounded-lg">
                            <div className="flex items-center gap-2 min-w-0">
                              <MapPin className="w-4 h-4 text-[#C3A36D] flex-shrink-0"/>
                              <span className="text-sm truncate">{loc.space_name}</span>
                            </div>
                            <Badge variant="outline" className="border-[#C3A36D]/30 text-[#C3A36D] flex-shrink-0">
                              {loc.scan_count}回
                            </Badge>
                          </div>)))}
                    </div>
          ...
          <TabsContent value="artworks" className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    ...
                    <div className="flex-1 w-full sm:w-auto">
                      <div className="relative">
                        ...
                        <Input placeholder="作品名、説明、IDで検索..." value={searchQuery} onChange={(e) => {
            ...
            setArtworkPage(1);
                    ...
                    <Button variant="outline" onClick={() => setIsFilterOpen(!isFilterOpen)} className="h-11 border-gray-200">
                    ...
                    <Button variant="outline" onClick={resetFilters} className="h-11 border-gray-200">
                      <X className="w-4 h-4 mr-2"/>
                      <span>リセット</span>
                    </Button>
                    
                    
                    <Button className="bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90 h-11" onClick={() => navigate("/signup/artist/artworks")}>
                  ...
                  <div className="flex flex-wrap gap-2 pb-2 border-b">
                    <Button variant={artworkFilter === "all" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("all");
            setArtworkPage(1);
        }} className={artworkFilter === "all" ? "bg-[#C3A36D] hover:bg-[#C3A36D]/90" : ""}>
                      すべて
                      {statusCounts != null && (<FilterChipCount n={statusCounts.all} active={artworkFilter === "all"}/>)}
                    </Button>
                    <Button variant={artworkFilter === "draft" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("draft");
            setArtworkPage(1);
        }} className={artworkFilter === "draft" ? "bg-gray-500 hover:bg-gray-600" : "border-gray-300"}>
                      未公開
                      {statusCounts != null && (<FilterChipCount n={statusCounts.draft} active={artworkFilter === "draft"}/>)}
                    </Button>
                    <Button variant={artworkFilter === "published" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("published");
            setArtworkPage(1);
        }} className={artworkFilter === "published" ? "bg-green-500 hover:bg-green-600" : "border-green-200"}>
                      オンライン公開中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.published} active={artworkFilter === "published"}/>)}
                    </Button>
                    <Button variant={artworkFilter === "exhibition_requested" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("exhibition_requested");
            setArtworkPage(1);
        }} className={artworkFilter === "exhibition_requested"
            ? "bg-orange-600 hover:bg-orange-700 text-white"
            : "border-orange-300"}>
                      展示依頼中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.exhibition_requested} active={artworkFilter === "exhibition_requested"}/>)}
                    </Button>
                    <Button variant={artworkFilter === ARTWORK_FILTER_IN_TRANSIT_UNIFIED
            ? "default"
            : "outline"} size="sm" onClick={() => {
            setArtworkFilter(ARTWORK_FILTER_IN_TRANSIT_UNIFIED);
            setArtworkPage(1);
        }} className={artworkFilter === ARTWORK_FILTER_IN_TRANSIT_UNIFIED
            ? "bg-violet-600 hover:bg-violet-700 text-white"
            : "border-violet-300"} title="①展示向け（アーティスト発送後）②法人返送（法人発送後）③回収返送（法人発送後）— いずれも輸送中。詳細は in_transit_kind">
                      輸送中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.in_transit} active={artworkFilter === ARTWORK_FILTER_IN_TRANSIT_UNIFIED}/>)}
                    </Button>
                    <Button variant={artworkFilter === "exhibited" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("exhibited");
            setArtworkPage(1);
        }} className={artworkFilter === "exhibited" ? "bg-[#C3A36D] hover:bg-[#C3A36D]/90" : "border-[#C3A36D]/30"}>
                      展示中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.exhibited} active={artworkFilter === "exhibited"}/>)}
                    </Button>
                    <Button variant={artworkFilter === ARTWORK_FILTER_RETURN_REQUESTED
            ? "default"
            : "outline"} size="sm" onClick={() => {
            setArtworkFilter(ARTWORK_FILTER_RETURN_REQUESTED);
            setArtworkPage(1);
        }} className={artworkFilter === ARTWORK_FILTER_RETURN_REQUESTED
            ? "bg-teal-600 hover:bg-teal-700 text-white"
            : "border-teal-300"} title="法人から返却が申請され、手続き中の作品（展示中のまま）">
                      返却申請中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.return_requested ?? 0} active={artworkFilter === ARTWORK_FILTER_RETURN_REQUESTED}/>)}
                    </Button>
                    <Button variant={artworkFilter === ARTWORK_FILTER_RETURNED_TO_ARTIST
            ? "default"
            : "outline"} size="sm" onClick={() => {
            setArtworkFilter(ARTWORK_FILTER_RETURNED_TO_ARTIST);
            setArtworkPage(1);
        }} className={artworkFilter === ARTWORK_FILTER_RETURNED_TO_ARTIST
            ? "bg-slate-600 hover:bg-slate-700 text-white"
            : "border-slate-300"} title="アーティストに到着済み（法人返却完了または回収完了。DB: recalled）">
                      アーティストに返却済み
                      {statusCounts != null && (<FilterChipCount n={statusCounts.recalled} active={artworkFilter === ARTWORK_FILTER_RETURNED_TO_ARTIST}/>)}
                    </Button>
                    <Button variant={artworkFilter === ARTWORK_FILTER_RECALL_REQUESTED
            ? "default"
            : "outline"} size="sm" onClick={() => {
            setArtworkFilter(ARTWORK_FILTER_RECALL_REQUESTED);
            setArtworkPage(1);
        }} className={artworkFilter === ARTWORK_FILTER_RECALL_REQUESTED
            ? "bg-rose-600 hover:bg-rose-700 text-white"
            : "border-rose-300"} title="アーティストが回収を依頼し、法人の発送を待っている状態（展示中のまま）">
                      回収依頼中
                      {statusCounts != null && (<FilterChipCount n={statusCounts.recall_requested ?? 0} active={artworkFilter === ARTWORK_FILTER_RECALL_REQUESTED}/>)}
                    </Button>
                    <Button variant={artworkFilter === "sold" ? "default" : "outline"} size="sm" onClick={() => {
            setArtworkFilter("sold");
            setArtworkPage(1);
        }} className={artworkFilter === "sold" ? "bg-blue-500 hover:bg-blue-600" : "border-blue-200"}>
                      売却済み
                      {statusCounts != null && (<FilterChipCount n={statusCounts.sold} active={artworkFilter === "sold"}/>)}
                    </Button>
                  </div>
                  
                  
                  <AnimatePresence>
                    {isFilterOpen && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="pt-4 space-y-6 border-t overflow-hidden">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid sm:grid-cols-4 gap-3">
                          ...
                          <Input type="date" value={filters.dateFrom} onChange={(e) => setFilters(prev => ({ ...prev, dateFrom: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" placeholder="開始日"/>
                          <Input type="date" value={filters.dateTo} onChange={(e) => setFilters(prev => ({ ...prev, dateTo: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" placeholder="終了日"/>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minPrice} onChange={(e) => setFilters(prev => ({ ...prev, minPrice: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxPrice} onChange={(e) => setFilters(prev => ({ ...prev, maxPrice: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                          <div className="space-y-2">
                            <Label className="text-xs text-gray-600">レンタル価格（円）</Label>
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minLeasePrice} onChange={(e) => setFilters(prev => ({ ...prev, minLeasePrice: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxLeasePrice} onChange={(e) => setFilters(prev => ({ ...prev, maxLeasePrice: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {sizeClassOptions.map((size) => (<div key={size} className="flex items-center space-x-2">
                                  <Checkbox id={`size-${size}`} checked={filters.sizeClass.includes(size)} onCheckedChange={() => toggleSizeClass(size)}/>
                          ...
                          <div className="grid sm:grid-cols-3 gap-4">
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <Input type="number" placeholder="最小" value={filters.minWidth} onChange={(e) => setFilters(prev => ({ ...prev, minWidth: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                                <Input type="number" placeholder="最大" value={filters.maxWidth} onChange={(e) => setFilters(prev => ({ ...prev, maxWidth: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              </div>
                            </div>
                            <div className="space-y-2">
                              <Label className="text-xs text-gray-600">高さ（cm）</Label>
                              <div className="grid grid-cols-2 gap-2">
                                <Input type="number" placeholder="最小" value={filters.minHeight} onChange={(e) => setFilters(prev => ({ ...prev, minHeight: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                                <Input type="number" placeholder="最大" value={filters.maxHeight} onChange={(e) => setFilters(prev => ({ ...prev, maxHeight: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              </div>
                            </div>
                            <div className="space-y-2">
                              <Label className="text-xs text-gray-600">奥行き（cm）</Label>
                              <div className="grid grid-cols-2 gap-2">
                                <Input type="number" placeholder="最小" value={filters.minDepth} onChange={(e) => setFilters(prev => ({ ...prev, minDepth: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                                <Input type="number" placeholder="最大" value={filters.maxDepth} onChange={(e) => setFilters(prev => ({ ...prev, maxDepth: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              </div>
                            </div>
                          </div>
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minWeight} onChange={(e) => setFilters(prev => ({ ...prev, minWeight: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxWeight} onChange={(e) => setFilters(prev => ({ ...prev, maxWeight: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {mediumOptions.map((option) => (<div key={option.value} className="flex items-center space-x-2">
                                  <Checkbox id={`medium-${option.value}`} checked={filters.medium.includes(option.value)} onCheckedChange={() => toggleMedium(option.value)}/>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {supportOptions.map((option) => (<div key={option.value} className="flex items-center space-x-2">
                                  <Checkbox id={`support-${option.value}`} checked={filters.support.includes(option.value)} onCheckedChange={() => toggleSupport(option.value)}/>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid sm:grid-cols-4 gap-3">
                          <Input type="number" placeholder="開始年" value={filters.yearFrom} onChange={(e) => setFilters(prev => ({ ...prev, yearFrom: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                          <Input type="number" placeholder="終了年" value={filters.yearTo} onChange={(e) => setFilters(prev => ({ ...prev, yearTo: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                          ...
                          <Switch id="hasFrame" checked={filters.hasFrame === true} onCheckedChange={(checked) => setFilters((prev) => ({
                ...prev,
                hasFrame: checked ? true : undefined,
                        ...
                        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                          ...
                          <Switch id="isAIGenerated" checked={filters.isAIGenerated === true} onCheckedChange={(checked) => setFilters((prev) => ({
                ...prev,
                isAIGenerated: checked ? true : undefined,
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {styleTagOptions.map((tag) => (<div key={tag} className="flex items-center space-x-2">
                              <Checkbox id={`tag-${tag}`} checked={filters.styleTags.includes(tag)} onCheckedChange={() => toggleStyleTag(tag)}/>
                              <Label htmlFor={`tag-${tag}`} className="text-sm cursor-pointer">
                                {tag}
                              </Label>
                            </div>))}
                        </div>
                        {filters.styleTags.length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
                            {filters.styleTags.map((tag) => (<Badge key={tag} variant="outline" className="border-[#C3A36D]/30 text-[#C3A36D]">
                                {tag}
                                <button onClick={() => toggleStyleTag(tag)} className="ml-2 hover:text-red-500">
                                  <X className="w-3 h-3"/>
                                </button>
                              </Badge>))}
                          </div>)}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid sm:grid-cols-3 gap-4">
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minViewCount} onChange={(e) => setFilters(prev => ({ ...prev, minViewCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxViewCount} onChange={(e) => setFilters(prev => ({ ...prev, maxViewCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                          <div className="space-y-2">
                            <Label className="text-xs text-gray-600">お気に入り数</Label>
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minFavoriteCount} onChange={(e) => setFilters(prev => ({ ...prev, minFavoriteCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxFavoriteCount} onChange={(e) => setFilters(prev => ({ ...prev, maxFavoriteCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                          <div className="space-y-2">
                            <Label className="text-xs text-gray-600">問い合わせ数</Label>
                            <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="最小" value={filters.minInquiryCount} onChange={(e) => setFilters(prev => ({ ...prev, minInquiryCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                              <Input type="number" placeholder="最大" value={filters.maxInquiryCount} onChange={(e) => setFilters(prev => ({ ...prev, maxInquiryCount: e.target.value }))} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {isLoadingArtworks ? (<Card className="bg-white">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredArtworks.map((artwork, index) => {
                ...
                return (<motion.div key={artwork.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="h-full">
                    <Card className="bg-white border-2 border-gray-200 hover:shadow-xl transition-shadow overflow-hidden h-full flex flex-col relative cursor-pointer" onClick={() => navigate(`/artwork-edit/${artwork.id}`)}>
                      
                      <div className="aspect-square bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center relative overflow-hidden" onMouseEnter={() => handleCarouselHover(artwork.id, true)} onMouseLeave={() => handleCarouselHover(artwork.id, false)}>
                        ...
                        <div className="absolute top-4 right-4 z-30 flex flex-col gap-2 items-end">
                          ...
                          {artwork.scans !== undefined && artwork.scans > 0 && (<Badge variant="outline" className="bg-white/95 border-gray-300 shadow-sm" title="作品ページの閲覧数（公開中に他ユーザーが詳細を開いた回数）">
                              <Eye className="w-3 h-3 mr-1 shrink-0"/>
                              閲覧 {artwork.scans}回
                            </Badge>)}
                          {(issueOpenCounts[artwork.id] ?? 0) > 0 && (<Badge className="bg-red-600 hover:bg-red-600 text-white border-0 shadow-md">
                        ...
                        {artwork.images && artwork.images.length > 0 ? (<>
                            ...
                            return (<motion.div key={imgIndex} initial={false} animate={{
                                    ...
                                    <ImageWithFallback src={imageUrl} alt={`${artwork.name} - 画像 ${imgIndex + 1}`} className="w-full h-full object-cover" fallback={<div className="w-full h-full flex items-center justify-center">
                            ...
                            }} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-1.5 sm:p-2 transition-all z-20" aria-label="前の画像">
                                  <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5"/>
                                </button>
                                <button type="button" onClick={(e) => {
                                e.stopPropagation();
                                handleCarouselNext(artwork.id);
                            }} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-1.5 sm:p-2 transition-all z-20" aria-label="次の画像">
                                  <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5"/>
                                </button>
                              </>)}
                            
                            
                            {artwork.images.length > 1 && (<div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
                                ...
                                {artworkCarousels.get(artwork.id)?.isAutoPlaying && !artworkCarousels.get(artwork.id)?.isHovering && (<div className="bg-black/50 text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
                                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white rounded-full animate-pulse"></div>
                                    <span className="hidden sm:inline">自動再生</span>
                                  </div>)}
                              </div>)}
                          </>) : artwork.isVideo ? (<Video className="w-20 h-20 text-gray-300" strokeWidth={1.5}/>) : (<ImageIcon className="w-20 h-20 text-gray-300" strokeWidth={1.5}/>)}
                      </div>
                      ...
                      <CardContent className="p-5 space-y-3 flex-1 flex flex-col">
                        ...
                        {artwork.status === "exhibited" && artwork.location && (<div className={`p-3 ${config.bgColor} rounded-lg`}>
                            <p className="text-sm text-gray-600 mb-1 flex items-center gap-1">
                              <MapPin className="w-3 h-3"/>
                              展示先
                            </p>
                            <p className="text-sm text-[#3A3A3A]">{artwork.location}</p>
                            {artwork.exhibitStart && (<p className="text-xs text-gray-500 mt-1">{artwork.exhibitStart}〜</p>)}
                          </div>)}

                        
                        {artwork.status === "sold" && (<div className={`p-3 ${config.bgColor} rounded-lg`}>
                        ...
                        navigate(`/artwork-edit/${artwork.id}`);
                          ...
                          {artwork.status === "exhibited" && (<Button variant="outline" size="sm" className="flex-1" onClick={(e) => handleRequestReturn(artwork, e)}>
            ...
            {artworkTotal > 0 && (<div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Select value={String(artworkPageSize)} onValueChange={(value) => {
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="icon" className="h-9 w-9" disabled={artworkPage <= 1} onClick={() => setArtworkPage((prev) => Math.max(1, prev - 1))}>
                    ...
                    <Button variant="outline" size="icon" className="h-9 w-9" disabled={artworkPage >= Math.max(1, Math.ceil(artworkTotal / artworkPageSize))} onClick={() => setArtworkPage((prev) => prev + 1 > Math.ceil(artworkTotal / artworkPageSize)
                    ? prev
                    : prev + 1)}>
            ...
            {!isLoadingArtworks && filteredArtworks.length === 0 && (<Card className="bg-white">
                <CardContent className="py-16 text-center">
                ...
                : `${statusConfig[artworkFilter as keyof typeof statusConfig]?.label}の作品はありません`}
                  ...
                  <Button className="bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90" onClick={() => navigate("/signup/artist/artworks")}>
          ...
          <TabsContent value="profile" className="space-y-6">
            <Card className="bg-white">
              <CardHeader>
                ...
                {profileData && (<div className="mt-2">
              ...
              <CardContent className="space-y-6">
                {isLoadingProfile ? (<div className="flex items-center justify-center py-8">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                      ...
                      {profileImage ? (<img src={profileImage} alt="プロフィール" className="w-24 h-24 rounded-full object-cover"/>) : (<User className="w-12 h-12 text-gray-400"/>)}
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => document.getElementById("profileImageInput")?.click()} disabled={isSavingProfile}>
                        <ImageIcon className="w-4 h-4"/>
                        <span>写真を変更</span>
                      </Button>
                      {profileImage && (<Button variant="outline" size="sm" onClick={handleDeleteProfileImageClick} disabled={isSavingProfile} className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200">
                          <X className="w-4 h-4"/>
                          <span>写真を削除</span>
                        </Button>)}
                    </div>
                    <input type="file" id="profileImageInput" className="hidden" accept="image/*" onChange={handleProfileImageChange} disabled={isSavingProfile}/>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                      ...
                      <Input id="name" value={profileFormData.name} onChange={(e) => setProfileFormData({ ...profileFormData, name: e.target.value })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                </div>

                
                <div className="space-y-2">
                      <Label htmlFor="email" className="text-sm">メールアドレス（非公開）</Label>
                      <Input id="email" type="email" value={profileData?.email || ""} disabled className="h-11 bg-gray-100 border-gray-200"/>
                ...
                <div className="space-y-2">
                      ...
                      <Input id="phone" type="tel" value={profileFormData.phone_number} onChange={(e) => setProfileFormData({ ...profileFormData, phone_number: e.target.value })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="shipping_postal_code" value={profileFormData.shipping_postal_code} onChange={(e) => setProfileFormData({
                ...profileFormData,
                shipping_postal_code: e.target.value,
            })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" placeholder="123-4567"/>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="shipping_prefecture" className="text-sm">
                        都道府県
                      </Label>
                      <Input id="shipping_prefecture" value={profileFormData.shipping_prefecture} onChange={(e) => setProfileFormData({
                ...profileFormData,
                shipping_prefecture: e.target.value,
            })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" placeholder="東京都"/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="shipping_city" value={profileFormData.shipping_city} onChange={(e) => setProfileFormData({ ...profileFormData, shipping_city: e.target.value })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="shipping_street_address" value={profileFormData.shipping_street_address} onChange={(e) => setProfileFormData({
                ...profileFormData,
                shipping_street_address: e.target.value,
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="shipping_building_name" value={profileFormData.shipping_building_name} onChange={(e) => setProfileFormData({
                ...profileFormData,
                shipping_building_name: e.target.value,
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="shipping_phone" type="tel" value={profileFormData.shipping_phone} onChange={(e) => setProfileFormData({ ...profileFormData, shipping_phone: e.target.value })} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="bio" rows={4} value={profileFormData.biography} onChange={(e) => setProfileFormData({ ...profileFormData, biography: e.target.value })} placeholder="あなたの作品について、制作のテーマやこだわりを教えてください" className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary resize-none"/>
                </div>

                
                <div className="space-y-2">
                      <Label htmlFor="career" className="text-sm">経歴・展示歴</Label>
                  <Textarea id="career" rows={4} value={profileFormData.career} onChange={(e) => setProfileFormData({ ...profileFormData, career: e.target.value })} placeholder="学歴、受賞歴、個展・グループ展の経歴など（例：2020年 東京藝術大学卒業）" className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary resize-none"/>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                          <Input id="instagram" value={profileFormData.instagram} onChange={(e) => setProfileFormData({ ...profileFormData, instagram: e.target.value })} placeholder="@username" className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                          {profileFormData.instagram && (<Button variant="outline" size="icon" onClick={() => {
                    const url = profileFormData.instagram.startsWith("http")
                        ? profileFormData.instagram
                        : `https://${profileFormData.instagram}`;
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                          <Input id="website" value={profileFormData.website} onChange={(e) => setProfileFormData({ ...profileFormData, website: e.target.value })} placeholder="https://" className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                          {profileFormData.website && (<Button variant="outline" size="icon" onClick={() => {
                ...
                <div className="flex gap-3 pt-4">
                  <Button className="bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90 flex-1 disabled:opacity-50 disabled:cursor-not-allowed" onClick={handleSaveProfile} disabled={isSavingProfile || !hasProfileChanges()}>
                    {isSavingProfile ? (<div className="flex items-center">
          ...
          <TabsContent value="revenue" className="space-y-6">
            ...
            <Card className="bg-white">
              ...
              <CardContent>
                {revenueAnalyticsLoading ? (<p className="text-sm text-gray-500">読み込み中…</p>) : revenueByMonth.length === 0 ? (<p className="text-sm text-gray-500">
                    販売済みの作品がまだないか、売上データがありません。
                  </p>) : (<div className="space-y-4">
                  {[...revenueByMonth].reverse().map((item, index) => (<div key={item.period} className="space-y-2">
                ...
                }} transition={{ delay: index * 0.05, duration: 0.5 }} className="h-full bg-gradient-to-r from-[#C3A36D] to-[#D4B478]"/>
                      </div>
                    </div>))}
                </div>)}
              </CardContent>
            ...
            <Card className="bg-white">
              ...
              <CardContent>
                {revenueAnalyticsLoading ? (<p className="text-sm text-gray-500">読み込み中…</p>) : !revenueAnalytics?.revenue_by_artwork?.length ? (<p className="text-sm text-gray-500">販売済みの作品はまだありません。</p>) : (<div className="space-y-4">
                  {revenueAnalytics.revenue_by_artwork.map((row) => (<div key={row.artwork_id} className="flex items-center justify-between p-4 bg-[#F8F6F1] rounded-lg">
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            {row.sold_at && (<p className="text-xs text-gray-500">
      ...
      <ArtistReturnRequestDialog open={returnRequestDialogOpen} onOpenChange={setReturnRequestDialogOpen} artwork={selectedArtworkForReturn}/>
      ...
      <AlertDialog open={deleteImageDialogOpen} onOpenChange={setDeleteImageDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteProfileImage} disabled={isSavingProfile} className="bg-red-600 hover:bg-red-700 focus:ring-red-600">
              {isSavingProfile ? (<>
