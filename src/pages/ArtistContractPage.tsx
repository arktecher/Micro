...
import { Heart, Palette, DollarSign, Package, Shield, Truck, AlertCircle, Coins, FileText, CheckCircle, } from "lucide-react";
...
import { peekFirstGoOnlineArtworkId, artworkOnlineConfirmPath, } from "@/lib/artworkGoOnlineFlow";
export function ArtistContractPage() {
    ...
    const keyPoints = [
        ...
        },
        {
            ...
            title: "価格はあなたが決めます",
            description: "作品の販売価格は、アーティストご本人が自由に設定していただけます。MGJが価格を一方的に変更することはございません。",
        },
        {
            icon: DollarSign,
            color: "text-accent",
            bgColor: "bg-accent/10",
            title: "売上の50％をお支払い",
            description: "作品が売れた際は、販売価格の50％をお支払いいたします。お支払いは3か月ごとにまとめて行われます。",
        },
        {
            icon: Package,
            color: "text-primary",
            bgColor: "bg-primary/10",
            title: "発送について",
            description: "作品は展示先（ホテル・オフィスなど）へ直接お送りいただきます。発送時の梱包箱はMGJからお送りしますので、同封のマニュアルに従って梱包をお願いいたします。",
        },
        {
            icon: Shield,
            color: "text-accent",
            bgColor: "bg-accent/10",
            title: "保険で大切に守ります",
            description: "展示中・輸送中の作品はMGJが加入する保険でカバーされます。万が一破損や紛失が発生した場合も、保険会社の定める条件に基づいて補償いたします。",
        },
        {
            icon: Truck,
            color: "text-primary",
            bgColor: "bg-primary/10",
            title: "返却時の送料について",
            description: "展示開始から3か月以内にアーティストが返却をご希望の場合は送料をご負担いただきます。3か月以降にマイクロギャラリー側が交換・返却する際は、先方が送料を負担いたします。",
        },
        {
            icon: AlertCircle,
            color: "text-gray-600",
            bgColor: "bg-gray-100",
            title: "作品の審査について",
            description: "作品の登録は原則自由ですが、他者の権利を侵害するものや、公的な空間への展示に適さないと判断される作品は、掲載をお断りする場合がございます。",
        },
        {
            icon: Coins,
            color: "text-gray-600",
            bgColor: "bg-gray-100",
            title: "将来的な料金について",
            description: "アーティストの皆さまからご利用料をいただくことは想定しておりません。将来的に法人向けのサービスに料金を導入する可能性がありますが、その場合も事前にお知らせいたします。",
        },
    ];
    return (<div className="min-h-screen bg-gradient-to-b from-cream/30 via-white to-gray-50">
      ...
      <div className="pt-20 sm:pt-24 pb-12 sm:pb-20">
        <div className="container mx-auto px-4 sm:px-6 max-w-5xl">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="mb-8 sm:mb-12">
            <Card className="shadow-xl border-2">
              ...
              <CardContent className="p-4 sm:p-6 md:p-8">
                <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
                  {keyPoints.map((point, index) => (<motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 * index }} className="flex gap-3 sm:gap-4">
                      <div className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full ${point.bgColor} flex items-center justify-center`}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="mb-8 sm:mb-12">
            <Card className="shadow-xl border-2">
              <CardHeader className="border-b bg-gradient-to-r from-gray-50 to-white px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4">
                <CardTitle className="text-lg sm:text-xl md:text-2xl text-center">
              ...
              <CardContent className="p-4 sm:p-6 md:p-8">
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4 p-4 sm:p-6 bg-gray-50 rounded-lg">
                    ...
                    <label htmlFor="agree" className="flex-grow text-sm sm:text-base text-gray-700 leading-relaxed cursor-pointer">
                      上記の「アーティスト利用契約（MGJアーティスト規約）」の内容を確認し、同意します。
                    </label>
                  </div>

                  
                  <Button onClick={handleAgree} disabled={!agreed} size="lg" className={`w-full sm:w-auto sm:min-w-[300px] md:min-w-[400px] h-12 sm:h-14 sm:h-16 text-sm sm:text-base md:text-lg mx-auto block flex items-center justify-center ${agreed
            ? "bg-primary hover:bg-primary/90"
            : "bg-gray-300 cursor-not-allowed"}`}>
                    {agreed ? (<div className="flex items-center justify-center gap-2">
