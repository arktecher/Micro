...
import { Building2, CreditCard, User, ArrowLeft, Shield, CheckCircle, AlertCircle, Palette, Clock, } from "lucide-react";
...
export function BankAccountEditPage() {
    ...
    const isFormValid = formData.bankName &&
        formData.branchName &&
        formData.accountNumber &&
        formData.accountHolderKana;
    ...
    return (<div className="min-h-screen bg-[#F8F6F1]">
      ...
      <div className="container mx-auto px-4 py-12 sm:py-16 md:py-24 max-w-3xl">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="mb-4 sm:mb-6">
          <Button variant="ghost" onClick={handleCancel} className="text-gray-600 hover:text-gray-900">
        ...
        {isSaved && (<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-4 sm:mb-6">
          ...
          <Card className="shadow-xl border-2 border-[#C3A36D]/20">
            ...
            <CardContent className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input id="bankName" type="text" placeholder="例：三菱UFJ銀行" value={formData.bankName} onChange={(e) => handleInputChange("bankName", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
              ...
              <div className="space-y-2">
                ...
                <Input id="branchName" type="text" placeholder="例：渋谷支店" value={formData.branchName} onChange={(e) => handleInputChange("branchName", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="accountNumber" type="text" placeholder={existingAccount ? "セキュリティのため、再度入力してください" : "例：1234567"} value={formData.accountNumber} onChange={(e) => {
            const value = e.target.value.replace(/\D/g, "");
            handleInputChange("accountNumber", value);
        }} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" maxLength={7} disabled={isSaving}/>
                <p className="text-xs sm:text-sm text-gray-500">
                  {existingAccount
            ...
            : "7桁の口座番号を入力してください"}
              ...
              <div className="space-y-2">
                ...
                <Input id="accountHolderKana" type="text" placeholder="例：ヤマダタロウ" value={formData.accountHolderKana} onChange={(e) => handleInputChange("accountHolderKana", e.target.value.toUpperCase())} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary"/>
                <p className="text-xs sm:text-sm text-gray-500">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 sm:pt-6">
                <Button onClick={handleSave} disabled={!isFormValid || isSaved || isSaving} className="bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90 flex-1 h-11 sm:h-12 text-sm sm:text-base disabled:opacity-50">
                  {isSaving ? (<>
                ...
                <Button variant="outline" onClick={handleCancel} disabled={isSaved || isSaving} className="flex-1 h-11 sm:h-12 text-sm sm:text-base">
                  キャンセル
                </Button>
              </div>
            </CardContent>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className="mt-6 sm:mt-8 text-center">
          <Card className="bg-white/50">
            <CardContent className="p-4 sm:p-6">
              <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">
                ご不明な点やご質問がございましたら、お気軽にお問い合わせください。
                <br className="hidden sm:block"/>
                <a href="mailto:support@microgalleryjapan.com" className="text-[#C3A36D] hover:underline break-all sm:break-normal">
