...
import { X, Package, CheckCircle2, AlertCircle, Truck, Calendar, MapPin, ArrowRight, } from "lucide-react";
...
export function ArtworkRecallPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                setLoadedImage(a.main_image_url ||
                ...
                try {
                    ...
                    if (cancelled)
                        return;
                    const onWall = ex.is_exhibited && ex.assignment?.status === "displaying";
                    if (!onWall) {
                        toast.info("回収の申請は、法人が作品を受領して展示が開始されたあとに可能です。");
                    ...
                    toast.info("展示情報を確認できませんでした。作品編集ページからご確認ください。");
                    navigate(`/artwork-edit/${artworkId}`);
                    return;
                }
            }
    ...
    if (isCompleted) {
        ...
        <div className="flex items-center justify-center p-4 sm:p-6 pt-20 sm:pt-24 pb-8 sm:pb-16">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="max-w-2xl w-full">
            <Card>
              <CardContent className="p-6 sm:p-12 text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: "spring", stiffness: 200 }} className="w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center shadow-lg">
                ...
                <div className="bg-[#C3A36D]/10 border border-[#C3A36D]/30 rounded-lg p-4 sm:p-6 mb-6 sm:mb-8 text-left">
                  ...
                  <ul className="space-y-2 text-xs sm:text-sm text-gray-700">
                    ...
                    {needsShippingFee && (<li className="flex items-start gap-2">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center items-stretch sm:items-center">
                  <Button size="lg" variant="default" onClick={() => artworkId && navigate(`/artwork-edit/${artworkId}`)} className="bg-[#C3A36D] hover:bg-[#C3A36D]/90 w-full sm:w-auto text-sm sm:text-base">
                    作品ページで状況を確認
                  </Button>
                  <Button size="lg" variant="outline" onClick={handleClose} className="w-full sm:w-auto text-sm sm:text-base">
                    ダッシュボードへ戻る
                  </Button>
                </div>
              </CardContent>
      ...
      <div className="pt-20 sm:pt-24 pb-8 sm:pb-16 px-4 sm:px-6">
        <div className="container mx-auto max-w-3xl space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="space-y-1 text-xs sm:text-sm">
                      {venueLabel && (<p className="text-gray-600 flex items-center gap-1">
                          <MapPin className="w-3 h-3 flex-shrink-0"/>
                          <span className="break-words">{venueLabel}</span>
                        </p>)}
                      {exhibitionDays > 0 && (<p className="text-gray-600 flex items-center gap-1">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
            <Card className={needsShippingFee ? "border-orange-300 bg-orange-50" : "border-green-300 bg-green-50"}>
              <CardContent className="p-4 sm:p-6">
                <div className="flex items-start gap-3">
                  {needsShippingFee ? (<AlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-orange-500 flex-shrink-0 mt-0.5"/>) : (<CheckCircle2 className="w-5 h-5 sm:w-6 sm:h-6 text-green-500 flex-shrink-0 mt-0.5"/>)}
                  <div className="flex-grow">
                    ...
                    {needsShippingFee ? (<div className="space-y-3">
                        <p className="text-xs sm:text-sm text-gray-700 leading-relaxed">
          ...
          {needsShippingFee && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
              <Card className="border-2 border-orange-200">
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-start gap-3">
                    <Checkbox id="agree-shipping" checked={agreeToShipping} onCheckedChange={(checked) => setAgreeToShipping(checked as boolean)} className="mt-1 flex-shrink-0"/>
                    <label htmlFor="agree-shipping" className="text-xs sm:text-sm text-gray-700 leading-relaxed cursor-pointer">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <Button variant="outline" size="lg" onClick={handleClose} className="flex-1 w-full sm:w-auto text-sm sm:text-base">
              キャンセル
            </Button>
            <Button size="lg" onClick={() => void handleRecall()} disabled={submitting || (needsShippingFee && !agreeToShipping)} className="flex-1 w-full sm:w-auto bg-[#C3A36D] hover:bg-[#C3A36D]/90 text-sm sm:text-base">
