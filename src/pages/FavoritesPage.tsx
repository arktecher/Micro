...
import {
  fetchFavorites,
  removeFavorite as removeCorporateFavorite,
...
export function FavoritesPage() {
  ...
  const loadFavorites = useCallback(async () => {
    ...
    try {
      if (userType === "corporate") {
        ...
        // Customer / artist: localStorage → fetch from API
  ...
  // Listen for external updates (e.g. toggled from another page)
  useEffect(() => {
  ...
  const handleRemove = async (artworkId: string) => {
    ...
    if (userType === "corporate") {
      ...
      const storageKey = getFavoritesKey(userType);
      const ids = (JSON.parse(localStorage.getItem(storageKey) || "[]") as string[]).filter(
        (id) => id !== artworkId,
      );
