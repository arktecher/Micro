              ...
              <CardContent className="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {photoPreview && (<div className="flex justify-center mb-3 sm:mb-4">
                        <div className="relative w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-2 sm:border-4 border-accent/20 shadow-lg">
                          <img src={photoPreview} alt="プロフィール写真プレビュー" className="w-full h-full object-cover"/>
                        </div>
                      </div>)}

                    <div className="flex items-center gap-2 sm:gap-4">
                      <label htmlFor="photo" className="flex items-center gap-2 px-3 sm:px-4 md:px-6 py-2 sm:py-3 border-2 border-dashed border-gray-300 rounded-lg hover:border-accent hover:bg-accent/5 transition-all cursor-pointer flex-1 sm:flex-initial">
                        <Upload className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 flex-shrink-0"/>
                        <span className="text-xs sm:text-sm text-gray-700 truncate">
                          {photo ? photo.name : "ファイルを選択"}
                        </span>
                      </label>
                      <Input id="photo" type="file" accept="image/*,.heic,.heif" onChange={handleFileChange} className="hidden"/>
                    </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {biographies.map((bio, index) => (<div key={index} className="flex flex-col sm:flex-row gap-2 sm:gap-3 items-stretch sm:items-start bg-gray-50 p-3 sm:p-4 rounded-lg">
                        <div className="w-full sm:w-28 md:w-32 flex-shrink-0">
                          <Select value={bio.year} onValueChange={(value) => updateBiography(index, "year", value)}>
                        ...
                        <div className="flex-grow min-w-0">
                          <Input type="text" placeholder="東京藝術大学 美術学部卒業" value={bio.content} onChange={(e) => updateBiography(index, "content", e.target.value)} className="h-10 sm:h-12 bg-white text-sm sm:text-base"/>
                        </div>

                        {biographies.length > 1 && (<Button type="button" variant="ghost" size="icon" onClick={() => removeBiography(index)} className="h-10 w-10 sm:h-12 sm:w-12 text-gray-400 hover:text-red-500 hover:bg-red-50 self-start sm:self-auto">
                            <X className="w-4 h-4 sm:w-5 sm:h-5"/>
                          </Button>)}
                      </div>))}

                    <Button type="button" variant="outline" onClick={addBiography} className="w-full border-dashed border-2 h-10 sm:h-12 hover:bg-accent/5 hover:border-accent text-sm sm:text-base">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="artworkDescription" placeholder="作品のスタイル、テーマ、使用する画材・技法などを教えてください。&#10;&#10;例：&#10;抽象画を中心に制作しています。自然の中にある色彩や形からインスピレーションを得て、アクリル絵の具を使った大胆な色使いが特徴です。" value={artworkDescription} onChange={(e) => setArtworkDescription(e.target.value)} className="min-h-[120px] sm:min-h-[150px] bg-white resize-y text-sm sm:text-base"/>
          ...
          <Card className="bg-white mt-4 sm:mt-6">
            <CardContent className="pt-4 sm:pt-6 px-4 sm:px-6">
              <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 sm:gap-0">
                <Button variant="outline" onClick={() => navigate("/signup/artist/artworks")} className="w-full sm:w-auto px-4 sm:px-6 order-2 sm:order-1 text-sm sm:text-base">
                  スキップして作品登録へ
                </Button>

                <Button onClick={handleSubmit} className="bg-primary hover:bg-primary/90 w-full sm:w-auto px-6 sm:px-8 order-1 sm:order-2 text-sm sm:text-base">
                  <span>保存して作品登録へ</span>
