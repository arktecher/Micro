...
import { CheckCircle, Sparkles, Package, QrCode, ArrowRight, Home, Plus } from "lucide-react";
...
export function ArtworkConfirmationPage() {
    ...
    if (!artwork || !space) {
        return (<div className="min-h-screen bg-gradient-to-b from-[#FAF9F6] to-white flex items-center justify-center">
    ...
    return (<div className="min-h-screen bg-gradient-to-b from-[#FAF9F6] via-white to-[#FAF9F6] relative overflow-hidden">
      <Header />

      {showConfetti && (<div className="fixed inset-0 pointer-events-none z-10">
          {[...Array(30)].map((_, i) => (<motion.div key={i} initial={{
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 relative z-20 pt-20 sm:pt-24">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Card className="bg-gradient-to-br from-white to-[#FAF9F6] border-4 border-[#C3A36D]/40 shadow-2xl overflow-hidden">
            ...
            damping: 10,
              ...
              <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-2xl sm:text-4xl text-[#3A3A3A] mb-3 sm:mb-4">
                🎉 おめでとうございます！
              </motion.h1>

              <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="text-lg sm:text-xl text-gray-700 mb-6 sm:mb-8">
                展示が確定しました
              </motion.p>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="max-w-2xl mx-auto">
                <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-6 bg-white/80 rounded-xl border-2 border-[#C3A36D]/20 mb-6 sm:mb-8">
                  <div className="w-24 h-24 sm:w-32 sm:h-32 flex-shrink-0">
                    <ImageWithFallback src={artwork.image} alt={artwork.title} className="w-full h-full object-cover rounded-lg border-2 border-[#C3A36D]/30"/>
                  </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="p-4 sm:p-6 bg-gradient-to-r from-[#C3A36D]/10 to-[#D4B478]/10 rounded-xl border border-[#C3A36D]/30 mb-6 sm:mb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="mt-6 sm:mt-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }} className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
          <Button onClick={() => navigate('/corporate-dashboard')} className="flex-1 h-12 sm:h-14 bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90 text-white text-sm sm:text-base">
            <Plus className="w-4 h-4 sm:w-5 sm:h-5"/>
            <span>他のスペースもアートを選ぶ</span>
          </Button>

          <Button onClick={() => navigate('/corporate-dashboard')} variant="outline" className="flex-1 h-12 sm:h-14 border-2 border-[#C3A36D] text-[#C3A36D] hover:bg-[#C3A36D]/5 text-sm sm:text-base">
            <Home className="w-4 h-4 sm:w-5 sm:h-5"/>
            <span>マイページへ戻る</span>
          </Button>
        </motion.div>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }} className="mt-6 sm:mt-8 p-4 sm:p-6 bg-blue-50 border border-blue-200 rounded-xl">
