...
import { Building2, TrendingUp, Eye, ShoppingCart, MapPin, Plus, Bell, Settings, LogOut, Sparkles, Calendar, ChevronRight, Image as ImageIcon, Users, BarChart3, Home, LifeBuoy, Wallet, ChevronDown, Brain, Loader2, Zap, Package, Truck, CheckCircle2, Clock, QrCode, RotateCcw, Trash2, Heart, Layers, } from "lucide-react";
...
import { listSpaces, getCorporateShippingDashboard, type SpaceResponse, type CorporateShippingIncomingItem, type CorporateShippingReturnItem, type CorporateShippingRecallItem, type CorporateShippingDisplayItem, } from "@/services/space.service";
import { artworkService, type Artwork } from "@/services/artwork.service";
import { fetchFavoriteIds, fetchFavorites, removeFavorite as removeFavoriteApi, } from "@/services/corporateFavorites.service";
...
const SPACE_CARD_FALLBACK_IMAGE = "https://images.unsplash.com/photo-1497366216548-37526070297c?w=800";
...
function mapSpaceResponseToCard(s: SpaceResponse): CorporateDashboardSpaceCard {
    ...
    const inReturnOrRecall = hasArtwork &&
...
function mapLocalSavedToCard(saved: Record<string, unknown>): CorporateDashboardSpaceCard {
    const images = saved.images as string[] | undefined;
    const hasArtwork = saved.currentArtwork != null && saved.currentArtwork !== false;
    return {
        id: String(saved.id),
...
const SHIPPING_FALLBACK_IMAGE = "https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=400";
...
export function CorporateDashboard() {
    ...
    const { canEdit: canEditCorporate, canAdmin: canAdminCorporate } = useCorporateOrgRole();
    const [activeTab, setActiveTab] = useState<CorporateDashboardTab>(() => typeof window !== "undefined" ? getCorporateTabFromHash() : "dashboard");
    ...
    const [shippingIncoming, setShippingIncoming] = useState<CorporateShippingIncomingItem[]>([]);
    const [shippingReturns, setShippingReturns] = useState<CorporateShippingReturnItem[]>([]);
    const [shippingRecalls, setShippingRecalls] = useState<CorporateShippingRecallItem[]>([]);
    const [shippingDisplayOverview, setShippingDisplayOverview] = useState<CorporateShippingDisplayItem[]>([]);
    const [shippingLoading, setShippingLoading] = useState(false);
    const [recommendedLoading, setRecommendedLoading] = useState(false);
    const [recommendedDisplayRows, setRecommendedDisplayRows] = useState<CorporateShippingDisplayItem[]>([]);
    const [recommendedFavoriteArtworks, setRecommendedFavoriteArtworks] = useState<Artwork[]>([]);
    ...
    const loadRecommendedTabData = useCallback(async () => {
        ...
        try {
            ...
            setRecommendedFavoriteArtworks(favItems.map((f) => ({
                id: f.id,
                title: f.title ?? "",
    ...
    const loadFavoritesTabData = useCallback(async () => {
        ...
        try {
            ...
            setFavoritesTabArtworks(favItems.map((f) => ({
                id: f.id,
                title: f.title ?? "",
                artist: f.artist_name ? { id: "", name: f.artist_name } : undefined,
                main_image_url: f.main_image_url ?? "",
    ...
    const loadSpaces = async () => {
        ...
        try {
            ...
            const raw = JSON.parse(localStorage.getItem("mgj_registered_spaces") || "[]") as Record<string, unknown>[];
    ...
    useEffect(() => {
        ...
        fetchFavoriteIds()
            ...
            .catch(() => {
            const saved = JSON.parse(localStorage.getItem("mgj_corporate_favorites") || "[]") as string[];
            setFavorites(saved);
        });
    ...
    useEffect(() => {
        const handleFavoritesUpdate = () => {
            const savedFavorites = JSON.parse(localStorage.getItem("mgj_corporate_favorites") || "[]") as string[];
    ...
    const expectedRevenue = monthRevenue * 0.1;
    return (<div className="flex min-h-screen flex-col bg-gray-50">
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
              <TabsList className="w-full justify-start border-b rounded-none h-auto p-0 bg-transparent min-w-max sm:min-w-0">
                <TabsTrigger value="dashboard" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
                  <BarChart3 className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0"/>
                  <span className="whitespace-nowrap">ダッシュボード</span>
                </TabsTrigger>
                <TabsTrigger value="spaces" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
                  <Home className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0"/>
                  <span className="whitespace-nowrap">スペース管理</span>
                </TabsTrigger>
                <TabsTrigger value="recommended" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
                  <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0"/>
                  <span className="whitespace-nowrap">作品一覧</span>
                </TabsTrigger>
                <TabsTrigger value="favorites" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
                  <Heart className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0"/>
                  <span className="whitespace-nowrap">お気に入り</span>
                </TabsTrigger>
                <TabsTrigger value="shipping" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
            ...
            0 && (<Badge className="ml-1 sm:ml-2 bg-accent text-white text-[10px] sm:text-xs px-1 sm:px-1.5 py-0">
                      {shippingIncoming.length +
                shippingReturns.length +
                shippingRecalls.length}
                    </Badge>)}
                </TabsTrigger>
                <TabsTrigger value="payment" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
                  <Wallet className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0"/>
                  <span className="whitespace-nowrap">入出金管理</span>
                </TabsTrigger>
                <TabsTrigger value="support" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-2 sm:py-3">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="container mx-auto flex min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-8">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="flex min-h-0 w-full flex-1 flex-col">
            ...
            <TabsContent value="dashboard" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
            ...
            borderRadius: "8px",
                            ...
                            <Legend />
                            <Line type="monotone" dataKey="views" stroke="#3b82f6" strokeWidth={2} name="QR閲覧数" dot={{ r: 4 }}/>
                            <Line type="monotone" dataKey="clicks" stroke="#8b5cf6" strokeWidth={2} name="クリック数" dot={{ r: 4 }}/>
                            <Line type="monotone" dataKey="sales" stroke="#10b981" strokeWidth={2} name="購入数" dot={{ r: 4 }}/>
                          </LineChart>
                        </ResponsiveContainer>
                  ...
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.5 }}>
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        {notifications.map((notification, index) => (<motion.div key={notification.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{
                duration: 0.3,
                delay: 0.6 + index * 0.1,
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {pastSalesArtworks.map((artwork, index) => (<motion.div key={artwork.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{
                duration: 0.3,
                delay: 0.7 + index * 0.1,
            }} className="flex items-center gap-4 p-4 border rounded-lg hover:shadow-md transition-all">
                          <ImageWithFallback src={artwork.image} alt={artwork.title} className="w-20 h-20 rounded-lg object-cover"/>
                          <div className="flex-grow">
            ...
            <TabsContent value="spaces" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {canEditCorporate ? (<Link to="/signup/corporate?addSpace=true">
                ...
                {isLoadingSpaces && (<div className="flex min-h-[min(28rem,calc(100vh-16rem))] flex-1 flex-col items-center justify-center gap-2 py-16 text-gray-600">
                    <Loader2 className="h-5 w-5 animate-spin text-primary"/>
                    <span>スペースを読み込み中...</span>
                  </div>)}

                {!isLoadingSpaces && allSpaces.length === 0 && (<Card className="border-dashed border-2 border-gray-200 bg-gray-50/50">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {allSpaces.map((space, index) => (<motion.div key={space.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="flex">
                      <Card className="bg-white hover:shadow-lg transition-all group overflow-hidden border-2 border-gray-200 hover:border-primary flex flex-col w-full cursor-pointer h-full" onClick={() => {
                ...
                navigate(`/corporate-space/${space.id}`, {
                    state: { space: fullSpace || space },
                        ...
                        <div className="relative aspect-video overflow-hidden">
                          <ImageWithFallback src={space.image || SPACE_CARD_FALLBACK_IMAGE} alt={space.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                          <div className="absolute top-3 right-3">
                            <Badge className={space.status === "展示中"
                ? "bg-green-500"
                : "bg-gray-500"}>
                              {space.status}
                            </Badge>
                        ...
                        <CardHeader className="flex-grow">
                          ...
                          <div className="space-y-1 text-sm text-gray-600">
                            ...
                            {space.artworks > 0 && (<div className="flex items-center gap-2 pt-2">
                        ...
                        <CardContent className="pt-0 space-y-2">
                          ...
                          {space.revenue > 0 && (<div className="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                  ...
                  {canEditCorporate ? (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: allSpaces.length * 0.1 }} className="flex">
                      <Card className="bg-white border-2 border-dashed border-primary/30 hover:border-primary hover:bg-primary/5 transition-all cursor-pointer flex flex-col items-center justify-center w-full" onClick={() => navigate("/signup/corporate?addSpace=true")}>
            ...
            <TabsContent value="recommended" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
              {recommendedLoading ? (<div className="flex min-h-[min(28rem,calc(100vh-16rem))] flex-1 flex-col items-center justify-center gap-3 py-16">
                  ...
                  <div className="lg:col-span-3 space-y-6">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }}>
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          {recommendedDisplayRows.length === 0 ? (<div className="text-center py-12 text-gray-500 text-sm">
                              <p className="mb-2">
                                展示中の作品はまだありません。
                              </p>
                              <p>
                              ...
                              <Button variant="outline" className="mt-4" onClick={() => handleTabChange("spaces")}>
                                スペース管理へ
                              </Button>
                            </div>) : (recommendedDisplayRows.map((row, index) => (<motion.div key={`${row.assignment_id}-${row.artwork_id}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{
                    duration: 0.5,
                    delay: index * 0.05,
                }} className="flex gap-4 p-4 rounded-lg border hover:shadow-lg transition-all cursor-pointer hover:border-primary" onClick={() => navigate(`/corporate-artwork/${row.artwork_id}`)}>
                                <div className="w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                                  <ImageWithFallback src={row.image || ""} alt={row.title} className="w-full h-full object-cover"/>
                                </div>
                                <div className="flex-grow min-w-0">
                                  <div className="flex items-start justify-between mb-2 gap-2">
                                    ...
                                    <div className="text-right shrink-0">
                                      <Badge variant="outline" className="mb-2 border-amber-200 bg-amber-50 text-amber-900">
                                        {row.pipeline_label}
                                      </Badge>
                                      <p className="text-lg text-accent mb-3">
                                        {row.price}
                                      </p>
                                      {canEditCorporate ? (<Button variant="outline" size="sm" disabled={row.pipeline_label !== "展示中"} title={row.pipeline_label !== "展示中"
                        ...
                        setReturnSelection({
                            ...
                            artwork: {
                                ...
                                artist: row.artist,
                                image: row.image ?? "",
                                displayedSince: new Date().toISOString(),
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }}>
                      <Card>
                        ...
                        <CardContent>
                          {recommendedFavoriteArtworks.length === 0 ? (<div className="text-center py-10 text-gray-500 text-sm space-y-3">
                              <p>まだお気に入りがありません。</p>
                              ...
                              <div className="flex flex-col sm:flex-row gap-2 justify-center pt-1">
                                ...
                                <Button variant="outline" onClick={() => handleTabChange("favorites")}>
                                  お気に入りタブへ
                                </Button>
                              </div>
                            </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              {recommendedFavoriteArtworks.map((artwork, index) => {
                    const img = artwork.main_image_url ||
                        artwork.images?.[0]?.image_url ||
                        "";
                    const priceStr = `¥${Math.round(Number(artwork.price) || 0).toLocaleString()}`;
                    return (<motion.div key={artwork.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{
                            duration: 0.4,
                            delay: index * 0.05,
                        }} className="border rounded-lg overflow-hidden hover:shadow-xl transition-all group cursor-pointer relative" onClick={() => navigate(`/artwork/${artwork.id}?source=site`)}>
                                      <div className="aspect-square bg-gray-100 overflow-hidden relative">
                                        <ImageWithFallback src={img} alt={artwork.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                                      </div>
                                      <div className="p-4 space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          {(artwork.style_tags || [])
                            .slice(0, 5)
                            .map((tag) => (<Badge key={tag} variant="secondary" className="text-xs">
                                                {tag}
                                              </Badge>))}
                                        </div>
                                      </div>
                  ...
                  <div className="lg:col-span-1 space-y-4">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        {recommendedArtworks.slice(0, 3).map((a) => (<button key={a.id} type="button" className="w-full text-left rounded-lg border p-2 hover:bg-muted/50 transition-colors" onClick={() => navigate(`/artwork/${a.id}?source=site`)}>
                        ...
                        {canEditCorporate ? (<Button variant="secondary" className="w-full" size="sm" onClick={() => setAiDialogOpen(true)}>
                            AI推薦の詳細
                          </Button>) : null}
                      </CardContent>
                    </Card>
            ...
            <TabsContent value="favorites" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  {favorites.length === 0 ? (<div className="text-center py-12 space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 justify-center">
                        ...
                        <Button variant="outline" onClick={() => handleTabChange("recommended")}>
                          作品一覧タブへ
                        </Button>
                      </div>
                    </div>) : favoritesTabLoading ? (<div className="flex min-h-[min(20rem,calc(100vh-20rem))] flex-col items-center justify-center gap-3 py-16">
                      <Loader2 className="h-10 w-10 animate-spin text-primary"/>
                      <p className="text-sm text-gray-600">
                        お気に入りを読み込み中…
                      </p>
                    </div>) : favoritesTabArtworks.length === 0 ? (<div className="text-center py-12 space-y-4">
                      <Heart className="w-16 h-16 text-gray-300 mx-auto"/>
                      <p className="text-gray-500 text-sm max-w-md mx-auto leading-relaxed">
                        お気に入りの作品をAPIから取得できませんでした。IDが古い、または非公開・削除された可能性があります。
                      </p>
                      <div className="flex flex-col sm:flex-row gap-2 justify-center">
                        <Button variant="outline" onClick={() => void loadFavoritesTabData()}>
                          再読み込み
                        </Button>
                        <Link to="/artworks">
                          <Button variant="default" className="gap-2">
                ...
                const img = artwork.main_image_url ||
                    ...
                    artwork.images?.sort((a, b) => a.image_order - b.image_order)[0]?.image_url ||
                    "";
                const priceStr = `¥${Math.round(Number(artwork.price) || 0).toLocaleString("ja-JP")}`;
                return (<Card key={artwork.id} className="cursor-pointer overflow-hidden transition-shadow hover:shadow-md" onClick={() => navigate(`/artwork/${artwork.id}?source=site`)}>
                            <div className="aspect-square relative overflow-hidden bg-gray-100">
                              <ImageWithFallback src={img} alt={artwork.title} className="w-full h-full object-cover"/>
                            </div>
                            <CardContent className="p-4">
                              ...
                              {(artwork.style_tags || []).length > 0 && (<div className="flex flex-wrap gap-1 mb-3">
                                  {(artwork.style_tags || [])
                            .slice(0, 5)
                            .map((tag) => (<Badge key={tag} variant="secondary" className="text-xs">
                                        {tag}
                              ...
                              {canEditCorporate ? (<Button variant="outline" className="w-full" onClick={(e) => {
                            e.stopPropagation();
            ...
            <TabsContent value="shipping" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
              {shippingLoading ? (<div className="flex min-h-[min(28rem,calc(100vh-16rem))] flex-1 flex-col items-center justify-center gap-3 py-16">
                  <Loader2 className="h-10 w-10 animate-spin text-primary"/>
                  <p className="text-sm text-gray-600">
                    配送情報を読み込み中...
                  </p>
                  ...
                  <div className="lg:col-span-3 space-y-6">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          {shippingDisplayOverview.length === 0 ? (<div className="text-center py-10 border border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                            ...
                            </div>) : (shippingDisplayOverview.map((item, index) => (<motion.div key={`${item.space_id}-${item.assignment_id}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{
                    duration: 0.4,
                    delay: index * 0.06,
                                ...
                                <div className="flex gap-4 p-4 bg-gradient-to-r from-violet-50 to-indigo-50">
                                  <div className="w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 border-2 border-white shadow">
                                    <ImageWithFallback src={item.image ?? SHIPPING_FALLBACK_IMAGE} alt={item.title} className="w-full h-full object-cover"/>
                                  </div>
                                  <div className="flex-grow min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                                      ...
                                      <div className="text-left sm:text-right flex-shrink-0">
                                        <Badge className={item.pipeline_label === "返送中"
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          {shippingIncoming.length === 0 ? (<div className="text-center py-12">
                            ...
                            </div>) : (shippingIncoming.map((item, index) => (<motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{
                    duration: 0.5,
                    delay: index * 0.1,
                                ...
                                <div className="flex gap-4 p-4 bg-gradient-to-r from-blue-50 to-cyan-50">
                                  ...
                                  <div className="flex-grow">
                                    <div className="flex items-start justify-between mb-2">
                                      ...
                                      <div className="text-right">
                                        <Badge className={item.status === "配送中"
                        ...
                        : "bg-green-500 text-white"}>
                                ...
                                <div className="p-4 bg-white space-y-3">
                                  ...
                                  <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.status === "配送中"
                    ? "bg-blue-500"
                    : item.status === "配送準備中"
                        ? "bg-orange-500"
                        : "bg-green-500"}`}>
                                      {item.status === "配送中" ? (<Truck className="w-5 h-5 text-white"/>) : item.status === "配送準備中" ? (<Package className="w-5 h-5 text-white"/>) : (<CheckCircle2 className="w-5 h-5 text-white"/>)}
                                    </div>
                                    <div className="flex-grow">
                                      <p className="text-sm text-gray-700">
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-gray-50 rounded-lg">
                                      ...
                                      <p className="text-sm text-gray-700">
                                        {formatShippingDashDate(item.order_date)}
                                      </p>
                                    </div>
                                    <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                                      ...
                                      <p className="text-sm text-green-700">
                                        {formatShippingDashDate(item.estimated_arrival)}
                                      </p>
                                    </div>
                                  </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.12 }}>
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          {shippingRecalls.length === 0 ? (<div className="text-center py-12">
                              <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                <Package className="w-8 h-8 text-gray-400"/>
                              </div>
                              <p className="text-gray-600 mb-2">
                            ...
                            </div>) : (shippingRecalls.map((item, index) => (<motion.div key={item.shipment_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{
                    duration: 0.5,
                    delay: index * 0.1,
                    ...
                    ? "bg-sky-600"
                    : "bg-amber-500"}`}>
                                      {item.status === "配送中" ? (<Truck className="w-5 h-5 text-white"/>) : (<Package className="w-5 h-5 text-white"/>)}
                                    </div>
                                    <div className="flex-grow">
                                      <p className="text-sm text-gray-700">
                                        {item.shipping_status}
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                                      ...
                                      <p className="text-sm text-blue-800">
                                        {formatShippingDashDate(item.estimated_return)}
                                      </p>
                                    </div>
                                  </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          {shippingReturns.length === 0 ? (<div className="text-center py-12">
                              <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            ...
                            </div>) : (shippingReturns.map((item, index) => (<motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{
                    duration: 0.5,
                    delay: index * 0.1,
                }} className="border-2 rounded-xl overflow-hidden hover:shadow-lg transition-all">
                                ...
                                <div className="flex gap-4 p-4 bg-gradient-to-r from-orange-50 to-red-50">
                                  ...
                                  <div className="flex-grow">
                                    <div className="flex items-start justify-between mb-2">
                                      ...
                                      <div className="text-right">
                                        <Badge className={item.status === "返送完了"
                    ? "bg-green-500 text-white"
                    : "bg-orange-500 text-white"}>
                                          {item.status}
                                        </Badge>
                                ...
                                <div className="p-4 bg-white space-y-3">
                                  ...
                                  <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.status === "返送完了"
                    ? "bg-green-500"
                    : "bg-orange-500"}`}>
                                      {item.status === "返送完了" ? (<CheckCircle2 className="w-5 h-5 text-white"/>) : (<RotateCcw className="w-5 h-5 text-white"/>)}
                                    </div>
                                    <div className="flex-grow">
                                      <p className="text-sm text-gray-700">
                                        {item.shipping_status}
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-gray-50 rounded-lg">
                                      ...
                                      <p className="text-sm text-gray-700">
                                        {formatShippingDashDate(item.return_date)}
                                      </p>
                                    </div>
            ...
            <TabsContent value="payment" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
              <div className="space-y-6">
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                    <Card className="h-full">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {canAdminCorporate ? (<Button variant="outline" className="w-full" onClick={() => setBankAccountDialogOpen(true)}>
                            口座情報を変更
                          </Button>) : null}
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                    <Card className="h-full">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {canAdminCorporate ? (<div className="space-y-2">
                            <Button variant="outline" className="w-full" onClick={() => setCardDialogOpen(true)}>
                              カード情報を変更
                            </Button>

                            <Button variant="outline" className="w-full text-red-600 hover:bg-red-50 hover:text-red-700 border-red-200" onClick={() => setCardDeleteConfirmOpen(true)}>
            ...
            <TabsContent value="support" className="mt-0 flex min-h-0 flex-1 flex-col outline-none">
      ...
      {canEditCorporate ? (<Dialog open={aiDialogOpen} onOpenChange={setAiDialogOpen}>
          <DialogContent>
            <DialogHeader>
      ...
      {canEditCorporate ? (<ArtworkReturnDialog open={returnSelection !== null} onOpenChange={(open) => {
                if (!open)
                    setReturnSelection(null);
            }} spaceId={returnSelection?.spaceId} onSuccess={() => void loadRecommendedTabData()} artwork={returnSelection?.artwork ?? null}/>) : null}

      
      {canAdminCorporate ? (<BankAccountDialog open={bankAccountDialogOpen} onOpenChange={setBankAccountDialogOpen}/>) : null}

      
      {canAdminCorporate ? (<CardInfoDialog open={cardDialogOpen} onOpenChange={setCardDialogOpen}/>) : null}

      
      {canAdminCorporate ? (<AlertDialog open={cardDeleteConfirmOpen} onOpenChange={setCardDeleteConfirmOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={handleCardDelete} className="bg-red-600 hover:bg-red-700">
                削除する
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
