...
import {
  ...
  type CatalogSortId,
} from "@/utils/artworkCatalogQuery";
import {
  readCatalogSearchParamsFromLocation,
  parseArtworksCatalogUrl,
  serializeArtworksCatalogUrl,
  type ArtworksCatalogUrlState,
} from "@/utils/artworksCatalogUrl";
...
export function ArtworksPage() {
  ...
  const initialCatalog = useMemo(
    () => parseArtworksCatalogUrl(readCatalogSearchParamsFromLocation()),
    []
  );
  ...
  const [debouncedArtistName, setDebouncedArtistName] = useState(
    initialCatalog.artistName
  );
  const [selectedSort, setSelectedSort] = useState<CatalogSortId>(
    initialCatalog.selectedSort
  );
  const [selectedColors, setSelectedColors] = useState<string[]>(
    initialCatalog.selectedColors
  );
  const [selectedSizes, setSelectedSizes] = useState<string[]>(
    initialCatalog.selectedSizes
  );
  const [selectedPriceRanges, setSelectedPriceRanges] = useState<string[]>(
    initialCatalog.selectedPriceRanges
  );
  const [selectedTechniques, setSelectedTechniques] = useState<string[]>(
    initialCatalog.selectedTechniques
  );
  const [selectedStyles, setSelectedStyles] = useState<string[]>(
    initialCatalog.selectedStyles
  );
  ...
  useEffect(() => {
    const onLocationChange = () => {
      ...
      if (incoming === lastPushedUrlRef.current) return;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(
            e instanceof Error ? e.message : "作品一覧の読み込みに失敗しました"
          );
          setArtworks([]);
          setTotalCount(0);
          setTotalPages(0);
        ...
        <div className="container mx-auto px-4 sm:px-6 max-w-[1600px]">
          ...
          >
            <SortOptions
              ...
              onSelect={(id) => setSelectedSort(id as CatalogSortId)}
            />
          </motion.div>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {!error && artworks.length === 0 ? (
                    <div className="flex min-h-[240px] flex-col items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50/80 px-4 py-12 text-center text-gray-600">
