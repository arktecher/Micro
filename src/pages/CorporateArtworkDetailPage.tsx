...
import { getCorporateDisplayArtworkContext, type CorporateDisplayArtworkContext, } from "@/services/space.service";
import { ArrowLeft, Eye, Calendar, MapPin, Package, AlertCircle, Clock, Share2, ChevronRight, ChevronLeft, RotateCcw, Loader2, } from "lucide-react";
...
export function CorporateArtworkDetailPage() {
    ...
    const load = useCallback(async () => {
        if (!id || !isAuthenticated || userType !== "corporate")
            return;
        setLoading(true);
        setError(null);
    ...
    const mainImage = useMemo(() => {
        ...
        if (artwork.images?.length) {
            const sorted = [...artwork.images].sort((a, b) => a.image_order - b.image_order);
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="container mx-auto flex min-h-0 flex-1 flex-col items-center justify-center px-4 pb-16 pt-24 max-w-lg text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              {artwork && (<Button onClick={() => navigate(`/artwork/${id}?source=site`)}>
                  作品ページを見る
                </Button>)}
            </div>
          </div>
    ...
    const showArtistRecallBanner = ctx.has_artist_recall_request && ctx.artist_recall_requested_at;
    return (<div className="flex min-h-screen flex-col bg-gray-50">
      <Header />
        ...
        <div className="container mx-auto flex min-h-0 flex-1 flex-col px-4 sm:px-6">
          <div className="mb-6 flex items-center justify-between gap-3">
            <nav className="flex min-w-0 flex-1 items-center gap-2 text-sm text-gray-600">
              <Link to="/corporate-dashboard" className="shrink-0 hover:text-primary transition-colors">
                ダッシュボード
              </Link>
              <ChevronRight className="w-4 h-4 shrink-0"/>
              <span className="truncate text-gray-900">{artwork.title}</span>
            </nav>
            <Button type="button" variant="ghost" size="sm" onClick={() => navigate(-1)} className="h-8 shrink-0 gap-1 px-2 text-xs sm:text-sm">
              <ArrowLeft className="h-3.5 w-3.5 sm:h-4 sm:w-4"/>
              戻る
            </Button>
          </div>

          {showArtistRecallBanner && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              <Card className="border-2 border-orange-300 bg-orange-50">
                ...
                : "返却手続きは編集者以上の権限をお持ちのメンバーが行えます。"}
                      ...
                      {canEditCorporate && (<Button onClick={handleReturn} className="bg-orange-600 hover:bg-orange-700 text-white gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <Card className="overflow-hidden p-0">
                  <div className="relative aspect-[4/5] w-full bg-muted/30">
                    <ImageWithFallback src={galleryImageUrls[galleryIndex] ??
            mainImage} alt={artwork.title} className="h-full w-full object-cover"/>
                    {galleryImageUrls.length > 1 && (<>
                        <button type="button" aria-label="前の画像" onClick={() => setGalleryIndex((i) => i <= 0
                ? galleryImageUrls.length - 1
                : i - 1)} className="absolute left-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-gray-200/80 bg-white/90 text-gray-800 shadow-sm backdrop-blur-sm transition hover:bg-white">
                          <ChevronLeft className="h-5 w-5"/>
                        </button>
                        <button type="button" aria-label="次の画像" onClick={() => setGalleryIndex((i) => i >= galleryImageUrls.length - 1 ? 0 : i + 1)} className="absolute right-2 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-gray-200/80 bg-white/90 text-gray-800 shadow-sm backdrop-blur-sm transition hover:bg-white">
                          <ChevronRight className="h-5 w-5"/>
                        </button>
                        <div className="absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 gap-1.5">
                          {galleryImageUrls.map((_, i) => (<button key={i} type="button" aria-label={`画像 ${i + 1} を表示`} aria-current={galleryIndex === i} onClick={() => setGalleryIndex(i)} className={cn("h-1.5 rounded-full transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80", galleryIndex === i
                    ? "w-5 bg-white shadow-sm"
                    : "w-1.5 bg-white/55 hover:bg-white/80")}/>))}
                        </div>
                      </>)}
                  </div>
                  {galleryImageUrls.length > 1 && (<div className="flex gap-2 overflow-x-auto border-t border-gray-100 bg-gray-50/90 p-2">
                      {galleryImageUrls.map((url, i) => (<button key={`${url}-${i}`} type="button" onClick={() => setGalleryIndex(i)} className={cn("relative h-14 w-14 shrink-0 overflow-hidden rounded-md border-2 transition-colors", galleryIndex === i
                    ? "border-primary ring-1 ring-primary/30"
                    : "border-transparent opacity-80 hover:opacity-100")}>
                          <ImageWithFallback src={url} alt={`${artwork.title} ${i + 1}`} className="h-full w-full object-cover"/>
                        </button>))}
                    </div>)}
                </Card>
              </motion.div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {(dimW > 0 || dimH > 0) && (<div className="flex items-center justify-between text-sm">
                          <span className="text-gray-600">サイズ</span>
                          <span className="text-gray-900">
                            {dimW} × {dimH} cm
                          </span>
                        </div>)}
                      {artwork.medium && (<div className="flex items-center justify-between text-sm">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => navigate(`/corporate-space/${ctx.space_id}`)} className="text-xs h-7 shrink-0">
                          スペース詳細
                          <ChevronRight className="w-3 h-3 ml-1"/>
                        </Button>
                      </div>
                      {ctx.facility_type && (<p className="text-xs text-gray-500 pl-6">{ctx.facility_type}</p>)}
                      {ctx.space_address && (<p className="text-xs text-gray-600 pl-6">{ctx.space_address}</p>)}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    {canEditCorporate && (<>
                        <Button variant="destructive" className="w-full" onClick={handleReturn}>
                          <RotateCcw className="w-4 h-4 mr-2"/>
                          返却する
                        </Button>
                        <Button variant="outline" className="w-full" onClick={() => navigate(`/artwork-issue-report/${ctx.space_id}`)}>
            ...
            <div className="lg:col-span-2 space-y-6">
              {artwork.description && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
                  <Card>
                    <CardHeader>
                ...
                <Card className="border-[#C3A36D]/30 bg-[#F8F6F1]/50">
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div className="flex items-start gap-3">
                      <div className={`w-2 h-2 rounded-full mt-1.5 ${ctx.display_days >= 180 ? "bg-green-500" : "bg-orange-500"}`}/>
                      <div className="flex-1">
                        <p className="text-gray-900 mb-1">
                          {ctx.shipping_cost_bearer === "artist" ? (<span className="text-green-700">
                              ✓ 返却送料：無料（展示期間6ヶ月以上）
                            </span>) : (<span className="text-orange-700">
                              返却送料：法人負担（展示期間6ヶ月未満）
                            </span>)}
                        </p>
            ...
            ` / あと${180 - ctx.display_days}日で無料`}
                    ...
                    {canEditCorporate && (<p className="text-gray-600 text-xs">
      ...
      {canEditCorporate && (<ArtworkReturnDialog open={returnDialogOpen} onOpenChange={setReturnDialogOpen} spaceId={ctx.space_id} onSuccess={load} artwork={{
                id: artwork.id,
                title: artwork.title,
