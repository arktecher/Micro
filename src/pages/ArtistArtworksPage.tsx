...
import { initGoOnlineQueue, artworkOnlineConfirmPath, } from "@/lib/artworkGoOnlineFlow";
...
export function ArtistArtworksPage() {
    ...
    if (!isInitialized) {
        return (<div className="min-h-screen bg-gradient-to-b from-cream/30 via-white to-gray-50 flex items-center justify-center">
    ...
    const handleFileChange = async (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            if (artwork) {
                ...
                const invalidFiles = filesArray.filter((file) => file.size > 50 * 1024 * 1024);
    ...
    const removeFile = async (artworkId: string, fileIndex: number) => {
        ...
        if (artwork) {
            ...
            const updatedFiles = artwork.files.filter((_, index) => index !== fileIndex);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(error.message || "作品の登録に失敗しました。もう一度お試しください。");
        }
        finally {
            setIsSubmitting(false);
        }
    };
    return (<div className="min-h-screen bg-gradient-to-b from-cream/30 via-white to-gray-50">
      <Header />
      ...
      <div className="pt-20 sm:pt-24 pb-12 sm:pb-20">
        <div className="container mx-auto px-4 sm:px-6 max-w-5xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
            {artworks.map((artwork, index) => (<motion.div key={artwork.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                <Card className="shadow-xl border-2 relative">
                  
                    {artworks.length > 1 && (<div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10">
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeArtwork(artwork.id)} className="h-8 w-8 sm:h-10 sm:w-10 text-gray-400 hover:text-red-500 hover:bg-red-50">
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-3 sm:p-4">
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-2">
                            ...
                            <span className="text-xs sm:text-sm text-gray-600">
                              {artwork.files.length >= 7 ? (<span className="text-green-600 font-semibold">
                                  ✓ 推奨枚数達成！
                                </span>) : (<span>推奨: 7〜8枚</span>)}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                            <div className={`h-full transition-all duration-300 ${artwork.files.length >= 7
                ? "bg-green-500"
                : "bg-accent"}`} style={{
                width: `${Math.min((artwork.files.length / 7) * 100, 100)}%`,
            }}></div>
                          </div>
                          {artwork.files.length < 7 && (<p className="text-xs text-gray-600 mt-2">
                              💡
                        ...
                        <div className="space-y-3 sm:space-y-4">
                          ...
                          {artwork.files.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-4">
                    ...
                    return (<div key={`${artwork.id}-${fileIndex}`} className="relative group bg-gray-100 rounded-lg overflow-hidden aspect-square border-2 border-gray-200 hover:border-accent transition-all">
                                    
                                    <div className="absolute inset-0">
                                      {isLoading ? (<div className="w-full h-full flex items-center justify-center bg-gray-200">
                                          <div className="text-center">
                                            <div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-2"></div>
                                            <p className="text-xs text-gray-600">読み込み中...</p>
                                          </div>
                                        </div>) : previewUrl ? (file.type.startsWith("video/") ? (<video src={previewUrl} className="w-full h-full object-cover"/>) : (<img src={previewUrl} alt={`Preview ${fileIndex + 1}`} className="w-full h-full object-cover" onError={(e) => {
                                ...
                                target.src = fallbackUrl;
                                  ...
                                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <Button type="button" variant="ghost" size="icon" onClick={() => removeFile(artwork.id, fileIndex)} className="text-white hover:text-red-500 hover:bg-white/20 h-8 w-8 sm:h-10 sm:w-10">
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-white text-xs p-1 sm:p-2 truncate">
                                    {file.name}
                                    {isHeicFile(file) && (<span className="ml-1 text-[10px] opacity-75">(HEIC)</span>)}
                                  </div>
                                </div>);
                })}
                            </div>)}
                          ...
                          <label htmlFor={`files-${artwork.id}`} className={`flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 px-4 sm:px-6 py-6 sm:py-8 border-2 border-dashed rounded-lg transition-all cursor-pointer ${artwork.files.length < 7
                ? "border-accent bg-accent/5 hover:bg-accent/10 hover:border-accent"
                : "border-gray-300 hover:border-primary hover:bg-primary/5"}`}>
                            <Upload className={`w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 ${artwork.files.length < 7
                ? "text-accent"
                : "text-gray-600"}`}/>
                            <div className="text-center">
                ...
                : artwork.files.length < 7
                    ? `さらに写真を追加（あと${7 - artwork.files.length}枚で推奨枚数）`
                    : "写真を追加"}
                              </span>
                          ...
                          <Input id={`files-${artwork.id}`} type="file" accept="image/*,video/*,.heic,.heif" multiple onChange={(e) => handleFileChange(artwork.id, e)} className="hidden"/>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 sm:space-y-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
                          <div className="space-y-2">
                            <Label htmlFor={`name-${artwork.id}`} className="text-sm">
                              作品名 <span className="text-accent">*</span>
                            </Label>
                            <Input id={`name-${artwork.id}`} type="text" placeholder="例：夏の思い出" value={artwork.name} onChange={(e) => updateArtwork(artwork.id, "name", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary text-sm sm:text-base"/>
                          </div>

                          <div className="space-y-2">
                            <Label htmlFor={`price-${artwork.id}`} className="text-sm">
                              販売価格（円）{" "}
                              <span className="text-accent">*</span>
                            </Label>
                            <Input id={`price-${artwork.id}`} type="number" placeholder="50000" value={artwork.price} onChange={(e) => updateArtwork(artwork.id, "price", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary text-sm sm:text-base"/>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid sm:grid-cols-3 gap-4 sm:gap-6">
                            <div className="space-y-2">
                              <Label htmlFor={`width-${artwork.id}`} className="text-xs sm:text-sm text-gray-600">
                                幅（cm）
                              </Label>
                              <Input id={`width-${artwork.id}`} type="number" step="0.1" placeholder="45.5" value={artwork.width} onChange={(e) => updateArtwork(artwork.id, "width", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary text-sm sm:text-base"/>
                            </div>

                            <div className="space-y-2">
                              <Label htmlFor={`height-${artwork.id}`} className="text-xs sm:text-sm text-gray-600">
                                高さ（cm）
                              </Label>
                              <Input id={`height-${artwork.id}`} type="number" step="0.1" placeholder="60.0" value={artwork.height} onChange={(e) => updateArtwork(artwork.id, "height", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary text-sm sm:text-base"/>
                            </div>

                            <div className="space-y-2">
                              <Label htmlFor={`depth-${artwork.id}`} className="text-xs sm:text-sm text-gray-600">
                                奥行き（cm）
                              </Label>
                              <Input id={`depth-${artwork.id}`} type="number" step="0.1" placeholder="3.0" value={artwork.depth} onChange={(e) => updateArtwork(artwork.id, "depth", e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary text-sm sm:text-base"/>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
                          <div className="space-y-2">
                            <Label htmlFor={`year-${artwork.id}`} className="text-sm">
                              制作年 <span className="text-accent">*</span>
                            </Label>
                            <Select value={artwork.year || undefined} onValueChange={(value) => updateArtwork(artwork.id, "year", value)}>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-4 h-10 sm:h-12">
                              <div className="flex items-center space-x-3 bg-white px-4 sm:px-6 py-2 sm:py-3 rounded-md border">
                                <Switch id={`hasFrame-${artwork.id}`} checked={artwork.hasFrame} onCheckedChange={(checked) => updateArtwork(artwork.id, "hasFrame", checked)}/>
                                <Label htmlFor={`hasFrame-${artwork.id}`} className="cursor-pointer text-sm sm:text-base">
                      ...
                      <StyleTagsSection selectedTags={artwork.styleTags} onTagsChange={(tags) => updateArtwork(artwork.id, "styleTags", tags)} aiRecommendedTags={[
                "抽象画",
                "ミニマル",
                "モダン",
                "温かみ",
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor={`theme-${artwork.id}`} className="text-sm">
                            作品の説明 <span className="text-accent">*</span>
                          </Label>
                          <Textarea id={`theme-${artwork.id}`} placeholder="この作品に込めた思いや、制作のきっかけ、テーマなどを自由にご記入ください。&#10;&#10;例：&#10;夏の海辺で感じた懐かしさと儚さを表現しました。波の音と潮の香り、子供の頃の記憶が重なり合う瞬間を色彩で表現しています。" value={artwork.theme} onChange={(e) => updateArtwork(artwork.id, "theme", e.target.value)} className="min-h-[120px] sm:min-h-[150px] bg-gray-100 border-gray-200 focus:bg-white focus:border-primary resize-none text-sm sm:text-base"/>
                          <p className="text-xs text-gray-500">
                            ...
                            <div className="space-y-3 sm:space-y-4">
                              ...
                              <div className="space-y-2 sm:space-y-3">
                                ...
                                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                                  ...
                                  <div className={`flex-1 px-4 sm:px-6 py-3 sm:py-4 rounded-lg border-2 transition-all cursor-pointer ${artwork.isAIGenerated
                ? "bg-purple-100 border-purple-500 shadow-md"
                : "bg-white border-gray-300 hover:border-gray-400"}`} onClick={() => updateArtwork(artwork.id, "isAIGenerated", true)}>
                                    <div className="flex items-center gap-2 sm:gap-3">
                                      <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${artwork.isAIGenerated
                ? "border-purple-600 bg-purple-600"
                : "border-gray-300"}`}>
                                        {artwork.isAIGenerated && (<div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white rounded-full"/>)}
                                      </div>
                                      <div className="flex-1 min-w-0">
                                        <div className={`text-sm sm:text-base ${artwork.isAIGenerated
                ? "text-purple-700"
                : "text-gray-700"}`}>
                                          はい
                                        </div>
                                        <div className={`text-xs ${artwork.isAIGenerated
                ? "text-purple-600"
                : "text-gray-500"}`}>
                                          AI技術を使用しています
                                        </div>
                                      </div>
                                      {artwork.isAIGenerated && (<Badge className="bg-purple-600 text-white text-xs sm:text-sm flex-shrink-0">
                                          AI
                                        </Badge>)}
                                    </div>
                                  </div>
                ...
                : "bg-white border-gray-300 hover:border-gray-400"}`} onClick={() => updateArtwork(artwork.id, "isAIGenerated", false)}>
                                    <div className="flex items-center gap-2 sm:gap-3">
                                      <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${!artwork.isAIGenerated
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.4 }} className="pt-4 sm:pt-6 border-t">
              ...
              <Button type="submit" size="lg" className="w-full sm:w-auto sm:min-w-[300px] h-12 sm:h-14 bg-primary hover:bg-primary/90 text-base sm:text-lg mx-auto flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" disabled={isSubmitting}>
                {isSubmitting ? (<>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }} className="mt-8 sm:mt-12 text-center">
            <Card className="bg-gradient-to-r from-cream/30 to-transparent border-0 shadow-none">
              <CardContent className="p-4 sm:p-6 md:p-8">
                <p className="text-sm sm:text-base text-gray-600 leading-relaxed px-2">
                  ...
                  <br className="hidden sm:block"/>
                  <a href="mailto:support@microgalleryjapan.com" className="text-primary hover:underline break-all sm:break-normal">
