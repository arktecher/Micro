...
import { ArrowLeft, AlertCircle, Camera, Upload, X, CheckCircle2, Phone, Mail, Loader2, } from "lucide-react";
import { getIssueReportContext, createIssueReport, uploadIssueReportImage, type IssueReportContext, } from "@/services/space.service";
import { useAuth } from "@/contexts/AuthContext";
import { corporateRoleAllowsEdit } from "@/lib/corporatePermissions";
const PLACEHOLDER_NO_ARTWORK_IMAGE = "data:image/svg+xml;charset=utf-8," +
    encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400" viewBox="0 0 400 400"><rect fill="#f3f4f6" width="400" height="400"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#9ca3af" font-family="system-ui,sans-serif" font-size="15">画像なし</text></svg>`);
...
export function ArtworkIssueReportPage() {
    ...
    useEffect(() => {
        ...
        getIssueReportContext(spaceId)
            ...
            if (!cancelled) {
                setCtx(null);
                setContextError(e instanceof Error ? e.message : "情報の取得に失敗しました");
            }
        })
            .finally(() => {
            if (!cancelled)
            ...
            navigate(`/artwork-issue-report-confirmation/${spaceId}`, {
                state: {
                    artwork: {
                        title: ctx.artwork_title || "無題",
                        artist: ctx.artist_name || "—",
                        image: (ctx.main_image_url && ctx.main_image_url.trim()) ||
    ...
    return (<div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <main className="flex min-h-0 flex-1 flex-col max-w-4xl mx-auto w-full px-4 sm:px-6 pt-24 sm:pt-28 pb-16">
        <div className="mb-6 sm:mb-8">
          <Button variant="ghost" onClick={() => navigate(`/corporate-space/${spaceId}`)} className="mb-4 gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <div className="w-full sm:w-40 h-40 rounded-lg overflow-hidden flex-shrink-0 mx-auto sm:mx-0">
                    <ImageWithFallback src={artworkCard.image} alt={artworkCard.title} className="w-full h-full object-cover"/>
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
            ...
            ["delivery", "配送時の破損"],
            ["other", "その他"],
        ] as const).map(([value, label]) => (<div key={value} className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-gray-50 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <Card>
              ...
              <CardContent>
                <input type="datetime-local" value={discoveryDate} onChange={(e) => setDiscoveryDate(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-sm sm:text-base"/>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
            <Card>
              ...
              <CardContent>
                <Textarea value={issueDescription} onChange={(e) => setIssueDescription(e.target.value)} placeholder="例：作品の左下に約3cmのひび割れを発見しました。昨日の時点では確認できなかったため、本日発生したものと思われます。" className="min-h-[120px] sm:min-h-[150px] resize-none"/>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
            <Card>
              ...
              <CardContent className="space-y-4">
                {images.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                    {images.map((image, index) => {
                const previewUrl = previewUrls.get(index) || URL.createObjectURL(image);
                return (<div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-gray-100">
                          <img src={previewUrl} alt={`Upload ${index + 1}`} className="w-full h-full object-cover" onError={(ev) => {
                        const target = ev.target as HTMLImageElement;
                        target.src = URL.createObjectURL(image);
                    }}/>
                          <button type="button" onClick={() => removeImage(index)} className="absolute top-2 right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-colors">
                            <X className="w-4 h-4"/>
                          </button>
                        </div>);
            })}
                  </div>)}
                {images.length < 5 && (<label className="block">
                    <input type="file" accept="image/*,.heic,.heif" multiple onChange={handleImageUpload} className="hidden"/>
            ...
            <Card className="border-red-200 bg-red-50/30">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 border-t border-red-200">
                  <div className="flex items-start gap-3">
                    <Checkbox id="report" checked={agreedToReport} onCheckedChange={(checked) => setAgreedToReport(checked as boolean)}/>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }} className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-end">
            <Button type="button" variant="outline" onClick={() => navigate(`/corporate-space/${spaceId}`)} disabled={isSubmitting} className="w-full sm:w-auto">
              キャンセル
            </Button>
            ...
            !discoveryDate ||
            !agreedToReport ||
            isSubmitting} className="bg-red-600 hover:bg-red-700 text-white min-w-[160px] w-full sm:w-auto">
              {isSubmitting ? (<>
