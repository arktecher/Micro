...
const faqData: FAQItem[] = [
    ...
    },
    {
        ...
        answer: "AIが空間の幅・天井高・視認距離から推奨サイズを算出し、複数候補を提示します。",
        category: "作品選定",
        relatedIds: ["q12", "q28"],
    },
    {
        ...
        answer: "はい。法人様はMGJ（日本法人）が窓口となり、アーティストとの個別契約は不要です。",
        category: "契約",
    },
    {
        id: "q17",
        question: "作品の入れ替えはどう調整しますか？",
        answer: "展示終了前にMGJから連絡し、\n— 継続展示\n— 別作品に入れ替え\n— 展示終了\nを選択いただきます。",
        category: "展示期間",
        relatedIds: ["q18", "q27"],
    },
    {
    ...
    },
    {
        ...
        answer: "MGJは輸送・展示期間の事故に対応する保険スキームを整備しています。\n通常利用の範囲での破損であれば、\n法人様に重大な過失がない限り、保険でカバーされる前提です。",
        category: "保険・破損",
        relatedIds: ["q20"],
    },
    {
    ...
    },
    {
        ...
        answer: "サービス改善・レコメンド精度向上目的で使用します。\n外部への販売や特定可能な形での提供は行いません。",
        category: "データ・セキュリティ",
    },
    {
        id: "q23",
    ...
    },
    {
        ...
        answer: "銀行振込またはクレジットカード決済に対応予定です。\n売上分配はMGJから登録口座へ振り込みます。",
        category: "料金・収益",
    },
    {
        id: "q26",
    ...
    },
    {
        ...
        answer: 'MGJは「オープンプラットフォーム方式」を採用しているため審査は行いません。\nただし、以下は自動的に非表示になります：\n— 暴力・差別表現\n— 著作権侵害\n— 公序良俗違反作品\n\nまた、法人ごとに「NGテイスト」が設定でき、\n表示しない傾向を調整できます。',
        ...
        <div className="max-w-[920px] mx-auto px-4 sm:px-6">
          ...
          <div className="mb-6 sm:mb-10">
            <div className="relative">
              ...
              <input type="text" placeholder="キーワードで検索（例：送料、サイズ、破損保証）" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-10 sm:h-11 pl-10 sm:pl-12 pr-4 border border-[#E5E5E5] rounded-lg focus:outline-none focus:border-primary transition-colors text-sm sm:text-base"/>
            </div>
          </div>
          ...
          <div className="mb-8 sm:mb-12 overflow-x-auto">
            <div className="flex gap-2 pb-2 min-w-max">
              {categories.map((category) => (<button key={category} onClick={() => setSelectedCategory(category)} className={`px-3 sm:px-4 py-2 rounded-lg border border-[#E5E5E5] text-xs sm:text-sm transition-colors whitespace-nowrap ${selectedCategory === category
          ...
          <div className="space-y-1">
            {filteredFAQs.length > 0 ? (<Accordion type="single" collapsible className="space-y-1">
                {filteredFAQs.map((faq) => (<AccordionItem key={faq.id} value={faq.id} id={faq.id} className="border border-[#E5E5E5] rounded-lg px-4 sm:px-6 overflow-hidden">
                    ...
                    <AccordionContent className="pb-4 sm:pb-6">
                      <div className="bg-[#FAFAFA] -mx-4 sm:-mx-6 px-4 sm:px-6 py-4 sm:py-5 text-[#5A5A5A] leading-relaxed text-sm sm:text-base">
                        {faq.answer.split("\n").map((line, index) => (<p key={index} className="mb-2 last:mb-0">
                            {line}
                          </p>))}

                        
                        {faq.relatedIds && faq.relatedIds.length > 0 && (<div className="mt-4 sm:mt-6 bg-[#F9F9F9] rounded-lg p-3 sm:p-4 border border-[#E5E5E5]">
                            <p className="text-xs text-[#6B6B6B] mb-2">関連する質問</p>
                            <div className="space-y-1">
                              {getRelatedQuestions(faq.relatedIds).map((related) => (<button key={related.id} onClick={() => scrollToQuestion(related.id)} className="block text-xs sm:text-sm text-primary hover:text-primary/80 transition-colors text-left">
                                  → {related.question}
                                </button>))}
                            </div>
                          </div>)}
              ...
              </Accordion>) : (<div className="text-center py-8 sm:py-12">
                <p className="text-sm sm:text-base text-[#6B6B6B]">
                  該当する質問が見つかりませんでした。
                  <br />
