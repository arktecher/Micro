...
/** Scroll list: always reserve gutter + thin visible scrollbar */
const ARTWORK_LIST_SCROLL_CLASS =
  "min-h-[280px] max-h-[min(60vh,640px)] overflow-y-scroll overscroll-y-contain px-4 sm:px-6 py-3 " +
...
export interface ManualArtworkSelectDialogProps {
  ...
  spaceId: string;
  /** Work currently on wall (displaying) */
  currentArtworkId?: string | null;
  currentArtworkTitle?: string | null;
  /** Another work already in exhibition-request pipeline for this space */
  pendingExhibitionArtworkId?: string | null;
  pendingExhibitionTitle?: string | null;
  /**
   * True when this space must not start another exhibition request (pipeline active).
   * Parent should set from GET space `pending_*`; blocks selection/assign even if ids are momentarily stale.
   */
  spaceBlocksNewExhibitionRequest?: boolean;
  onAssigned: () => void | Promise<void>;
}

export function ManualArtworkSelectDialog({
  open,
  onOpenChange,
  spaceId,
  currentArtworkId,
  ...
  spaceBlocksNewExhibitionRequest = false,
  onAssigned,
}: ManualArtworkSelectDialogProps) {
      ...
      try {
        if (listTab === "all") {
          const res = await artworkService.listArtworks({
            ...
            sort_order: "desc",
            /** Not on any wall; excludes exhibition_requested / in_transit etc. via status=published */
          ...
          const computed =
            typeof tp === "number" && tp >= 1
              ? tp
              : Math.max(1, Math.ceil((res.total || 0) / PAGE_SIZE));
          setTotalPages(computed);
        } else {
          const raw = JSON.parse(
            localStorage.getItem(STORAGE_KEYS.CORPORATE_FAVORITES) || "[]",
          );
          const ids: string[] = Array.isArray(raw)
            ? raw.map(String).slice(0, 40)
            : [];
          const results = await Promise.all(
            ids.map((id) => artworkService.getArtwork(id).catch(() => null)),
          );
          let pubs = results.filter(
            (x): x is Artwork =>
              x != null &&
              /* API uses status exhibited for on-wall; exhibition_requested etc. are non-published */
              x.status === "published",
          );
        ...
        if (!cancelled) {
          ...
          setTotalPages(1);
          toast.error(
            e instanceof Error ? e.message : "作品一覧の取得に失敗しました",
          );
        }
      } finally {
  ...
  const handleAssign = async () => {
    if (!selected) return;
    if (spaceBlocksHardBlock) return;
    if (pendingPipelineConflict) return;
    setAssigning(true);
    try {
      ...
      console.error(e);
      toast.error(
        e instanceof Error ? e.message : "展示依頼の送信に失敗しました",
      );
    } finally {
      setAssigning(false);
    }
  ...
  /** Wall shows another work — new selection starts a new exhibition *request* (artist ships first). */
  const replacingWallDisplay =
    Boolean(currentArtworkId) &&
    String(currentArtworkId) !== String(selected?.id);

  /** Space already has a different work in pending / in_transit pipeline */
  const pendingPipelineConflict = Boolean(
    pendingExhibitionArtworkId &&
      selected &&
      String(pendingExhibitionArtworkId) !== String(selected.id),
  );

  const sameAsPendingPipeline = Boolean(
    pendingExhibitionArtworkId &&
      selected &&
      String(pendingExhibitionArtworkId) === String(selected.id),
  );

  /** True when GET space says pipeline is active and selection is not the in-flight work (cannot assign). */
  const spaceBlocksHardBlock =
    Boolean(spaceBlocksNewExhibitionRequest) &&
    Boolean(selected) &&
    !(
      pendingExhibitionArtworkId &&
      String(pendingExhibitionArtworkId) === String(selected.id)
        ...
        {step === "pick" ? (
          <>
            <div className="px-4 sm:px-6 py-3 space-y-3 shrink-0 border-b border-border/60">
              {spaceBlocksNewExhibitionRequest && (
                <p className="text-[11px] sm:text-xs rounded-md bg-red-50 border border-red-200 text-red-900 px-3 py-2 leading-relaxed">
                  このスペースではすでに作品の展示手続き（依頼〜輸送中）が進行中です。完了するまで別の作品は選べません。
                </p>
              )}
              {pendingExhibitionArtworkId && (
                <p className="text-[11px] sm:text-xs rounded-md bg-amber-50 border border-amber-200 text-amber-950 px-3 py-2 leading-relaxed">
                  {pendingExhibitionTitle ? (
                    <>
            ...
            <div className={cn("flex-1 min-h-0 border-y border-border/50", ARTWORK_LIST_SCROLL_CLASS)}>
              <div className="space-y-2.5 pb-1">
                {listLoading ? (
                  <div className="flex justify-center py-14 text-muted-foreground text-sm gap-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (
                          ...
                          toast.error(
                            "このスペースでは別の展示手続きが進行中です。完了してから作品を選び直してください。",
                          );
                          return;
                        }
                        setSelected(a);
          ...
          selected && (
            <div className="px-4 sm:px-6 py-4 space-y-4 max-h-[min(55vh,520px)] overflow-y-auto [scrollbar-width:thin] [scrollbar-color:rgba(0,0,0,0.28)_#e5e7eb] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300">
              {pendingPipelineConflict && (
                <div className="text-xs sm:text-sm rounded-lg bg-red-50 border border-red-200 text-red-900 px-3 py-2">
                  進行中の展示依頼:
                  {pendingExhibitionTitle ? (
                    <span className="font-medium">「{pendingExhibitionTitle}」</span>
                  ) : (
                    "別の作品"
                  )}
              ...
              {sameAsPendingPipeline && !pendingPipelineConflict && (
                <div className="text-xs sm:text-sm rounded-lg bg-slate-50 border border-slate-200 text-slate-800 px-3 py-2">
              ...
              {replacingWallDisplay &&
                currentArtworkTitle &&
                !pendingPipelineConflict && (
                  <div className="text-xs sm:text-sm rounded-lg bg-amber-50 border border-amber-200 text-amber-900 px-3 py-2">
          ...
          {step === "confirm" ? (
            <>
              ...
              <Button
                ...
                disabled={
                  assigning || pendingPipelineConflict || spaceBlocksHardBlock
                }
