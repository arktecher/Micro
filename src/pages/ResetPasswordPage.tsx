    ...
    if (isSuccess) {
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-12 sm:py-16">
          <div className="w-full max-w-md text-center">
            ...
            <Button onClick={() => navigate("/login-selection")} className="bg-primary hover:bg-primary/90 text-white">
              ログインへ
            </Button>
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            {error && (<div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Link to="/forgot-password" className="text-xs text-red-600 hover:underline mt-1 inline-block">
                    新しいリセットメールをリクエスト
                  </Link>
                </div>
              </div>)}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input id="password" type="password" placeholder="8文字以上" value={password} onChange={(e) => setPassword(e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required minLength={8}/>
              ...
              <div className="space-y-2">
                ...
                <Input id="confirmPassword" type="password" placeholder="パスワードを再入力" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="h-11 bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required minLength={8}/>
                {confirmPassword && password !== confirmPassword && (<p className="text-xs text-red-500">
                    パスワードが一致しません
                  </p>)}
              </div>

              <Button type="submit" className="w-full h-11 sm:h-12 bg-primary hover:bg-primary/90 text-white" disabled={isSubmitting || password !== confirmPassword}>
                {isSubmitting ? (<>
            ...
            <div className="mt-8 text-center">
              <Link to="/forgot-password" className="text-sm text-gray-600 hover:text-primary transition-colors">
                リセットリンクを再送信
              </Link>
            </div>
          </motion.div>
