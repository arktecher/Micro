...
export function SpaceSelectionModal({ open, onOpenChange }: SpaceSelectionModalProps) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        {spaces.length === 0 ? (<div className="text-center py-12">
          ...
          </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            {spaces.map((space) => (<Card key={space.id} className="cursor-pointer hover:shadow-lg transition-all hover:border-primary group" onClick={() => handleSelectSpace(space.id)}>
                <CardContent className="p-0">
                  <div className="aspect-video relative overflow-hidden">
                    <ImageWithFallback src={space.image} alt={space.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                  </div>
                  <div className="p-4">
                    ...
                    {space.displayedArtworks > 0 && (<p className="text-xs text-gray-500 mt-2">
