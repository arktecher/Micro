...
import { ArrowLeft, TrendingUp, TrendingDown, Download, Calendar, Filter, Package, ShoppingCart, CreditCard } from "lucide-react";
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {filteredTransactions.map((transaction) => (<div key={transaction.id} className={`p-3 sm:p-4 rounded-lg border transition-all hover:shadow-md ${transaction.type === "income"
                ? "bg-green-50 border-green-200"
                : "bg-gray-50 border-gray-200"} ${transaction.status === "pending" ? "opacity-60" : ""}`}>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${transaction.type === "income" ? "bg-green-100" : "bg-gray-100"}`}>
                        {getTypeIcon(transaction.type)}
                      </div>
                      <div className="flex-grow min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className="text-xs sm:text-sm font-medium">{transaction.description}</p>
                          {transaction.status === "pending" && (<Badge variant="outline" className="text-xs">
                              予定
                            </Badge>)}
                        </div>
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs text-gray-500">
                          ...
                          {transaction.artworkTitle !== "-" && (<span className="truncate">「{transaction.artworkTitle}」</span>)}
                        </div>
                      </div>
                    </div>
                    <div className="text-left sm:text-right flex-shrink-0">
                      <p className={`text-base sm:text-lg font-semibold ${transaction.type === "income" ? "text-green-600" : "text-gray-600"}`}>
