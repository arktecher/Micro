...
import { CheckCircle, Sparkles, ImageIcon, Video, Home, Eye, Heart, Star, Tag, Clock, } from "lucide-react";
...
export function ArtworkPublishPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (cancelled)
                    return;
                const mine = results.filter((a): a is Artwork => a !== null && a.artist_id === currentUser.id);
    ...
    return (<div className="min-h-screen bg-[#F8F6F1]">
      ...
      {isPublishing ? (<section className="min-h-screen flex items-center justify-center px-4 pt-20 sm:pt-24">
          ...
          <section className="relative pt-20 sm:pt-24 md:pt-32 pb-12 sm:pb-16 md:pb-20 px-4 overflow-hidden bg-gradient-to-b from-white via-[#F8F6F1]/50 to-[#F8F6F1]">
            ...
            <div className="container mx-auto max-w-4xl relative z-10">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
                ...
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: "spring", stiffness: 200 }} className="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-[#C3A36D] to-[#D4B478] rounded-full mb-6 sm:mb-8 shadow-2xl">
                  <CheckCircle className="w-12 h-12 sm:w-14 sm:h-14 text-white" strokeWidth={2.5}/>
                </motion.div>

                <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }} className="text-3xl sm:text-4xl md:text-5xl text-[#3A3A3A] mb-4 sm:mb-6 px-2">
                  作品の公開が完了しました
                </motion.h1>

                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.8 }} className="text-lg sm:text-xl text-gray-600 mb-4 leading-relaxed px-2">
          ...
          <section className="py-8 sm:py-12 px-4 bg-white border-y border-gray-200">
            <div className="container mx-auto max-w-5xl">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0, duration: 0.6 }} className="text-center mb-8 sm:mb-10">
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.6 }} className="text-center p-6 sm:p-8 bg-gradient-to-br from-[#F8F6F1] to-white rounded-xl sm:rounded-2xl border border-[#C3A36D]/20">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.3, duration: 0.6 }} className="text-center p-6 sm:p-8 bg-gradient-to-br from-[#F8F6F1] to-white rounded-xl sm:rounded-2xl border border-[#C3A36D]/20">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-[#C3A36D]/10 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5, duration: 0.6 }} className="text-center p-6 sm:p-8 bg-gradient-to-br from-[#F8F6F1] to-white rounded-xl sm:rounded-2xl border border-[#C3A36D]/20 sm:col-span-2 md:col-span-1">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-[#C3A36D]/10 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
          ...
          <section className="py-12 sm:py-16 px-4 bg-[#F8F6F1]">
            <div className="container mx-auto max-w-6xl">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.6, duration: 0.6 }} className="text-center mb-8 sm:mb-12">
                <h2 className="text-2xl sm:text-3xl text-[#3A3A3A] mb-3 sm:mb-4">
                  {publishedArtworks.length > 0
                ? "今回オンライン公開した作品"
                : "ギャラリーについて"}
              ...
              {isLoading ? (<Card className="p-8 sm:p-12 text-center bg-white rounded-xl sm:rounded-2xl">
                  <CardContent>
                ...
                </Card>) : publishedArtworks.length === 0 ? (<Card className="p-8 sm:p-12 text-center bg-white rounded-xl sm:rounded-2xl">
                  <CardContent>
                    ...
                    <Button onClick={() => navigate("/dashboard#artworks")} className="bg-[#C3A36D] hover:bg-[#C3A36D]/90 rounded-xl">
                      マイページへ
                    </Button>
                  </CardContent>
                </Card>) : (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
                  {publishedArtworks.map((artwork: any, index: number) => (<motion.div key={artwork.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.8 + index * 0.1, duration: 0.6 }}>
                      <Card className="bg-white rounded-xl sm:rounded-2xl overflow-hidden border-2 border-[#C3A36D]/20 shadow-lg hover:shadow-xl transition-shadow relative">
                        ...
                        <div className="aspect-square bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center overflow-hidden relative">
                          {artwork.main_image_url ? (<ImageWithFallback src={artwork.main_image_url} alt={artwork.name} className="w-full h-full object-cover" fallback={<ImageIcon className="w-16 h-16 sm:w-20 sm:h-20 text-gray-300" strokeWidth={1.5}/>}/>) : artwork.isVideo ? (<Video className="w-16 h-16 sm:w-20 sm:h-20 text-gray-300" strokeWidth={1.5}/>) : (<ImageIcon className="w-16 h-16 sm:w-20 sm:h-20 text-gray-300" strokeWidth={1.5}/>)}
                        </div>
                        ...
                        <CardContent className="p-4 sm:p-5 space-y-2 sm:space-y-3">
                          ...
                          {artwork.tags && artwork.tags.length > 0 && (<div className="flex flex-wrap gap-1.5 sm:gap-2">
                              {artwork.tags.map((tag: string, i: number) => (<Badge key={i} variant="outline" className="text-xs border-[#C3A36D]/30 text-[#C3A36D]">
                          ...
                          <div className="text-xs sm:text-sm text-gray-600 space-y-1">
                            {artwork.width > 0 && artwork.height > 0 && (<p>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.0, duration: 0.6 }} className="flex justify-center items-center">
                <Button onClick={handleGoToDashboard} size="lg" className="bg-gradient-to-r from-[#3A3A3A] to-[#4A4A4A] hover:opacity-90 text-white px-6 sm:px-10 py-5 sm:py-7 text-base sm:text-lg rounded-xl sm:rounded-2xl shadow-xl w-full sm:w-auto">
          ...
          <section className="py-12 sm:py-16 px-4 bg-white">
            <div className="container mx-auto max-w-4xl">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.2, duration: 0.6 }} className="bg-gradient-to-br from-[#F8F6F1] to-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 border-2 border-[#C3A36D]/20 text-center">
