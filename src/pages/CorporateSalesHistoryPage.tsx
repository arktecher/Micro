...
import { TrendingUp, ShoppingCart, MapPin, Calendar, Download, ArrowLeft, DollarSign, CheckCircle, } from "lucide-react";
...
export function CorporateSalesHistoryPage() {
    ...
    const filteredSales = salesHistory.filter((sale) => {
        if (filter === "all")
            return true;
        if (filter === "paid")
            return sale.payoutStatus === "支払済";
        if (filter === "pending")
            return sale.payoutStatus === "処理中";
        if (filter === "unpaid")
            return sale.payoutStatus === "未払い";
        return true;
    });
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 sm:px-6 pt-20 sm:pt-24 pb-8 sm:pb-12">
        ...
        <div className="mb-6 sm:mb-8">
          <Button variant="ghost" onClick={() => navigate("/corporate-dashboard")} className="mb-4 text-sm sm:text-base">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-6 sm:mb-8">
          <Card>
            ...
            fontSize: "12px",
                    ...
                    <Bar dataKey="revenue" fill="#10b981" name="収益（円）" radius={[8, 8, 0, 0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <Card>
            ...
            <CardContent className="space-y-4">
              {filteredSales.length === 0 ? (<div className="text-center py-12">
                  <p className="text-gray-500">該当する販売履歴がありません</p>
                </div>) : (filteredSales.map((sale, index) => (<motion.div key={sale.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + index * 0.05 }} className="flex flex-col sm:flex-row gap-4 p-4 rounded-lg border hover:shadow-md transition-all">
                    <div className="w-full sm:w-24 h-48 sm:h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                      <ImageWithFallback src={sale.image} alt={sale.artworkTitle} className="w-full h-full object-cover"/>
                    </div>
                    <div className="flex-grow min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-2 gap-2">
                        ...
                        <div className="text-left sm:text-right flex-shrink-0">
                          <Badge className={`mb-2 ${sale.payoutStatus === "支払済"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="mt-6 sm:mt-8 p-4 sm:p-6 bg-blue-50 border border-blue-200 rounded-xl">
          ...
          <ul className="space-y-2 text-xs sm:text-sm text-gray-700">
            ...
            <li>
              • 振込先は
              <Link to="/corporate-profile?tab=payment" className="text-blue-600 underline ml-1">
                設定ページ
              </Link>
              で変更できます
            </li>
