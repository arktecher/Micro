          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div className="space-y-2">
                ...
                <Input id="name" type="text" placeholder="山田 太郎" value={formData.name} onChange={(e) => handleChange("name", e.target.value)} className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="email">メールアドレス</Label>
                <Input id="email" type="email" placeholder="example@email.com" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="password">パスワード</Label>
                <Input id="password" type="password" placeholder="8文字以上" value={formData.password} onChange={(e) => handleChange("password", e.target.value)} className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required minLength={8}/>
              ...
              <div className="space-y-2">
                ...
                <Input id="confirmPassword" type="password" placeholder="再度入力してください" value={formData.confirmPassword} onChange={(e) => handleChange("confirmPassword", e.target.value)} className="bg-gray-100 border-gray-200 focus:bg-white focus:border-primary" required minLength={8}/>
              </div>

              
              <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                <Checkbox id="terms" checked={formData.agreeToTerms} onCheckedChange={(checked) => handleChange("agreeToTerms", checked as boolean)} required/>
                <label htmlFor="terms" className="text-sm text-gray-700 leading-relaxed cursor-pointer">
                  <Link to="/privacy-policy" className="text-[#8B7355] hover:underline">
                    プライバシーポリシー
                  </Link>
                  および
                  <Link to="/artist-terms" className="text-[#8B7355] hover:underline ml-1">
                    利用規約
                  </Link>
                  に同意します
                </label>
              ...
              <Button type="submit" className="w-full bg-[#222] hover:bg-[#333] text-white h-11 sm:h-12" disabled={!formData.agreeToTerms || isSubmitting}>
                {isSubmitting ? (<>
            ...
            <div className="mt-6 sm:mt-8 text-center">
              <p className="text-sm text-gray-600">
                すでにアカウントをお持ちの方は
                <Link to="/login/customer" className="text-[#8B7355] hover:underline ml-1">
                  ログイン
                </Link>
              </p>
            </div>
