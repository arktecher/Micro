...
import { Globe, ImageIcon, Clock, Loader2, CheckCircle, Sparkles, Frame, Lightbulb, ArrowRight, ChevronLeft, ChevronRight, } from "lucide-react";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { artworkService, type Artwork, type OnlineConfirmContext, } from "@/services/artwork.service";
import { useAuth } from "@/contexts/AuthContext";
import { toast } from "sonner";
import { ImageWithFallback } from "@/components/common/ImageWithFallback";
import { advanceGoOnlineQueue, addOnboardingPublishedArtworkId, } from "@/lib/artworkGoOnlineFlow";
...
export function ArtworkGoOnlinePage() {
    ...
    const [onlineContext, setOnlineContext] = useState<OnlineConfirmContext | null>(null);
    ...
    useEffect(() => {
        if (imageUrls.length <= 1)
            return;
        if (!carousel.isAutoPlaying || carousel.isHovering)
            return;
    ...
    const handleCarouselPrev = useCallback(() => {
        ...
        setCarousel((c) => ({
            ...c,
            currentIndex: (c.currentIndex - 1 + imageUrls.length) % imageUrls.length,
            isAutoPlaying: false,
        }));
        scheduleResumeAutoPlay();
    }, [imageUrls.length, scheduleResumeAutoPlay]);
    ...
    const onCarouselTouchEnd = useCallback((e: TouchEvent) => {
        const start = touchStartXRef.current;
        touchStartXRef.current = null;
        if (start == null || imageUrls.length <= 1)
            return;
    ...
    const canPublishFromContext = !onlineContext ||
    ...
    const handleYes = async () => {
        ...
        try {
            ...
            if (String(confirmed.status) !== "published") {
                throw new Error("オンライン公開の反映を確認できませんでした。ダッシュボードで作品の状態をご確認ください。");
            }
            ...
            const msg = e instanceof Error ? e.message : "公開に失敗しました。もう一度お試しください";
            toast.error(msg);
        }
        finally {
    ...
    return (<div className="min-h-screen bg-[#F8F6F1]">
      ...
      <section className="relative py-10 sm:py-16 px-4 overflow-hidden bg-gradient-to-b from-white via-[#F8F6F1]/50 to-[#F8F6F1]">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="container mx-auto max-w-3xl text-center relative z-10">
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} transition={{ delay: 0.1, type: "spring", stiffness: 200 }} className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-[#C3A36D] to-[#D4B478] rounded-full mb-6 shadow-xl">
      ...
      <section className="relative py-10 sm:py-14 px-4 pb-28">
        <div className="container mx-auto max-w-lg">
          {isLoading ? (<Card className="p-10 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
              <CardContent className="p-0">
                <Loader2 className="w-10 h-10 text-[#C3A36D] mx-auto mb-4 animate-spin"/>
                <p className="text-gray-600">作品を読み込んでいます…</p>
              </CardContent>
            </Card>) : loadError || !artwork ? (<Card className="p-10 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            </Card>) : (<motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
              ...
              <Card className="overflow-hidden bg-white rounded-2xl border-2 border-[#C3A36D]/20 shadow-xl mb-8">
                ...
                <div key={artwork.id} role={imageUrls.length > 1 ? "region" : undefined} aria-roledescription={imageUrls.length > 1 ? "carousel" : undefined} aria-label={imageUrls.length > 1 ? `${artwork.title}の画像ギャラリー` : undefined} className="relative w-full max-w-[min(100%,min(56vh,420px))] overflow-hidden mx-auto touch-pan-y bg-gradient-to-br from-gray-50 to-gray-100" onMouseEnter={() => handleCarouselHover(true)} onMouseLeave={() => handleCarouselHover(false)} onTouchStart={onCarouselTouchStart} onTouchEnd={onCarouselTouchEnd}>
                  {imageUrls.length > 0 ? (<>
                      <div className="relative w-full pb-[100%] h-0">
                        <div className="absolute inset-0">
                          {imageUrls.map((imageUrl, imgIndex) => {
                    const isActive = carousel.currentIndex === imgIndex;
                    return (<motion.div key={`${imageUrl}-${imgIndex}`} initial={false} animate={{
                            opacity: isActive ? 1 : 0,
                            scale: isActive ? 1 : 0.98,
                        }} transition={{
                            duration: 0.45,
                            ease: "easeInOut",
                        }} className={`absolute inset-0 ${isActive ? "z-10" : "z-0 pointer-events-none"}`} aria-hidden={!isActive}>
                                <ImageWithFallback src={imageUrl} alt={`${artwork.title} - 画像 ${imgIndex + 1} / ${imageUrls.length}`} className="h-full w-full object-cover select-none" draggable={false}/>
                              </motion.div>);
                })}
                        </div>
                      </div>
                      ...
                      {imageUrls.length > 1 && (<>
                        ...
                        handleCarouselNext();
                          ...
                          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-20 max-w-[calc(100%-1rem)]">
                            <div className="flex items-center justify-center gap-1.5 px-1" role="tablist" aria-label="画像の選択">
                              {imageUrls.map((_, i) => (<button key={i} type="button" role="tab" aria-selected={carousel.currentIndex === i} aria-label={`画像 ${i + 1} を表示`} onClick={(e) => {
                            e.stopPropagation();
                            goToSlide(i);
                        }} className={`h-2 rounded-full transition-all ${carousel.currentIndex === i
                            ? "w-6 bg-white"
                            ...
                            <div className="flex items-center gap-2 flex-wrap justify-center">
                              ...
                              {carousel.isAutoPlaying && !carousel.isHovering && (<div className="bg-black/50 text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
                ...
                <CardContent className="p-5 sm:p-6">
                  ...
                  {artwork.status === "published" && (<p className="text-sm text-green-600 mt-2">すでに公開済みです</p>)}
                  {publishHint && artwork.status !== "published" && (<div role="alert" className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                      <p className="font-medium">公開の条件を満たしていません</p>
                      <p className="mt-1 text-amber-800/90">{publishHint}</p>
                      {onlineContext?.missing_fields &&
                    onlineContext.missing_fields.length > 0 && (<p className="mt-2 text-xs text-amber-800/80">
                            不足:{" "}
                            {onlineContext.missing_fields
                        .map((f) => f === "title"
                        ? "タイトル"
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <Button type="button" variant="outline" size="lg" className="flex-1 rounded-xl border-gray-300 h-12 sm:h-14 text-base" disabled={isSubmitting} onClick={handleNo}>
                  今は公開しない
                </Button>
                <Button type="button" size="lg" className="flex-1 rounded-xl bg-gradient-to-r from-[#C3A36D] to-[#D4B478] hover:opacity-90 text-white h-12 sm:h-14 text-base shadow-lg disabled:opacity-50" disabled={isSubmitting || !canPublishFromContext} onClick={handleYes}>
                  {isSubmitting ? (<>
